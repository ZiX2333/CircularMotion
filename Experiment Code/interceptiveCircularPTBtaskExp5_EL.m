...
% Add blocks, rest time, pesudorandom trails, experiment trials and control
% trials, Xuan, Aug 3
...
rng('shuffle');
num_Cond = 3; % 3dir (clockwise or counterclockw or stable) * velocity * ecc
num_loc = 8; %
num_block = 3;
...
for condi = 1:num_block
    for i = 1:num_trial
        % Sti.IniAng(i) = 2*pi*rand(1);
        % Sti.LocYs(i) = round(Sti.Distance * sin(randi(Sti.IniAngRange)) * ppd ) + CenterY;
        % generate following sequence:
        IniAngCur = Sti.IniAng(i,condi);
        % Generate trials based on preallocated condition sequence
        if  rem(num_CondSec(i,condi),3) == 1 % counterclockwise
            for j = 1:(FP.OffFrames(i)+Sti.SacFrames+Sti.SmPFrames)
                Sti.PathXs{condi}(i,j) = Sti.Distance* cos(IniAngCur)*ppd(1) + CenterX;
                Sti.PathYs{condi}(i,j) = Sti.Distance* sin(IniAngCur)*ppd(2) + CenterY;
                Sti.PathAngs{condi}(i,j) = IniAngCur;
                IniAngCur = IniAngCur + Sti.Speed*ifi;
            end
        elseif rem(num_CondSec(i,condi),3) == 2 %clockwise
            for j = 1:(FP.OffFrames(i)+Sti.SacFrames+Sti.SmPFrames)
                Sti.PathXs{condi}(i,j) = Sti.Distance* cos(IniAngCur)*ppd(1) + CenterX;
                Sti.PathYs{condi}(i,j) = Sti.Distance* sin(IniAngCur)*ppd(2) + CenterY;
                Sti.PathAngs{condi}(i,j) = IniAngCur;
        ...
        elseif rem(num_CondSec(i,condi),3) == 0 %stable
            for j = 1:(FP.OffFrames(i)+Sti.SacFrames+Sti.SmPFrames)
                Sti.PathXs{condi}(i,j) = Sti.Distance* cos(IniAngCur)*ppd(1) + CenterX;
                Sti.PathYs{condi}(i,j) = Sti.Distance* sin(IniAngCur)*ppd(2) + CenterY;
                Sti.PathAngs{condi}(i,j) = IniAngCur;
    ...
    for i_trial = 1:num_trial
        ...
        if state_Delay_flag == 1
            ...
            if FP.OffFrames == 0
                ...
                for i_frame = 1:FP.OffFrames(i_trial,i_block)
                    ...
                    Screen('DrawDots', w1, [Sti.PathXs{i_block}(i_trial,i_frame), Sti.PathYs{i_block}(i_trial,i_frame)],...
                    ...
                    % Write message to EDF file to mark the start time of stimulus presentation
                    Eyelink('Message', ['FP_POS X ',num2str(FP.LocX), ', Y ', num2str(FP.LocY)]);
                    Eyelink('Message', ['Targ_POS X ',num2str(Sti.PathXs{i_block}(i_trial,i_frame)), ', Y ', ...
                        num2str(Sti.PathYs{i_block}(i_trial,i_frame)), ', Ang ', num2str(Sti.PathAngs{i_block}(i_trial,i_frame))]);
                    % Eyelink('Message', 'Targ_POS X %f, Y %f, Ang %f', Sti.PathXs{i_block}(i_trial,i_frame), Sti.PathYs{i_block}(i_trial,i_frame), Sti.PathAngs{i_block}(i_trial,i_frame));
                    ...
                    EyeData.trial{i_trial, i_block}.State{StateNo}.frame{i_frame}.targX = Sti.PathXs{i_block}(i_trial,i_frame);
                    EyeData.trial{i_trial, i_block}.State{StateNo}.frame{i_frame}.targY = Sti.PathYs{i_block}(i_trial,i_frame);
                    EyeData.trial{i_trial, i_block}.State{StateNo}.string = '';
        ...
        if state_SacResp_flag == 1
            ...
            for i_frame = 1: Sti.SacFrames
                ...
                % draw stimulus
                Screen('DrawDots', w1, [Sti.PathXs{i_block}(i_trial,i_frame1), Sti.PathYs{i_block}(i_trial,i_frame1)],...
                    Sti.sizePixel(1), Sti.Color, [], 2);
                ...
                Eyelink('Message', ['Targ_POS X ',num2str(Sti.PathXs{i_block}(i_trial,i_frame)), ', Y ', ...
                    num2str(Sti.PathYs{i_block}(i_trial,i_frame)), ', Ang ', num2str(Sti.PathAngs{i_block}(i_trial,i_frame))]);
                % Eyelink('Message', 'Targ_POS X %f, Y %f, Ang %f', Sti.PathXs{i_block}(i_trial,i_frame), Sti.PathYs{i_block}(i_trial,i_frame),Sti.PathAngs{i_block}(i,j));
                ...
                EyeData.trial{i_trial, i_block}.State{StateNo}.frame{i_frame}.eyeX = eyeX;
                EyeData.trial{i_trial, i_block}.State{StateNo}.frame{i_frame}.eyeY = eyeY;
                EyeData.trial{i_trial, i_block}.State{StateNo}.frame{i_frame}.targX = Sti.PathXs{i_block}(i_trial,i_frame1);
                EyeData.trial{i_trial, i_block}.State{StateNo}.frame{i_frame}.targY = Sti.PathYs{i_block}(i_trial,i_frame1);
                EyeData.trial{i_trial, i_block}.State{StateNo}.string = '';
                ...
                if sqrt(((FP.LocX-eyeX)/ppd(1)).^2 + ((FP.LocY - eyeY)/ppd(2)).^2)  > FP.WindowSize / 2
                    ...
                    if sqrt(((Sti.PathXs{i_block}(i_trial,i_frame1)-eyeX)/ppd(1)).^2 + ((Sti.PathYs{i_block}(i_trial,i_frame1) - eyeY)/ppd(2)).^2)  <= Sti.WindowSize / 2
