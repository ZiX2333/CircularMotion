...
rng('shuffle');
num_Cond = 3; % 3dir (inward, outward, stationary) * velocity * ecc 1 inward, 2 outward, 3 stationary
% % num_loc = 8; %
num_block = 3;
...
FP.OffDurRange = [0,500]; % time after stimulus osnet, FP disappear
%FP.Preduration = 600;
...
Sti.DisRange = [-25,20; -2,-30; -18,-3];
Sti.DirMove = [1,-1]; % moving inward or o utward
Sti.Speed = 30; %30 deg/s 
...
for i = 1:num_Cond
    if i ~=num_Cond
        Sti.IniDis(num_CondSec==i) = Sti.DisRange(i,1);
    elseif i ==num_Cond
        Sti.IniDis(num_CondSec==i) = randi(Sti.DisRange(i,:), sum(sum(num_CondSec == i)), 1);
    end
end
...
for iBlock = 1:num_block
    for i = 1:num_trial
        ...
        if  rem(num_CondSec(i,iBlock),3) == 1 % inward
            for j = 1:(FP.OffFrames(i,iBlock)+Sti.SEnFrames+Sti.SacFrames+Sti.SmPFrames)
                ...
                % let it stop at the ending limit, smooth p to a certain
                % limit location
                if CurrDis + Sti.Speed*ifi <= Sti.DisRange(1,2)
                    CurrDis = CurrDis + Sti.Speed*ifi;
                elseif CurrDis + Sti.Speed*ifi > Sti.DisRange(1,2)
                    CurrDis = CurrDis;
                end
            end
        elseif rem(num_CondSec(i,iBlock),3) == 2 %clockwise
            for j = 1:(FP.OffFrames(i,iBlock)+Sti.SEnFrames+Sti.SacFrames+Sti.SmPFrames)
                Sti.PathXs{i,iBlock}(j) = CurrDis*ppd(1) + CenterX;
                % Sti.PathYs{i,iBlock}(j) = CurrDis*ppd(2) + CenterY;
                Sti.PathYs{i,iBlock}(j) = CenterY; % Horizontal leftward, vertical component not moving
                ...
                if CurrDis - Sti.Speed*ifi >= Sti.DisRange(2,2)
                    CurrDis = CurrDis - Sti.Speed*ifi;
                elseif CurrDis - Sti.Speed*ifi < Sti.DisRange(2,2)
                    CurrDis = CurrDis;
                end
            end
        elseif rem(num_CondSec(i,iBlock),3) == 0 %stable
            for j = 1:(FP.OffFrames(i,iBlock)+Sti.SEnFrames+Sti.SacFrames+Sti.SmPFrames)
                Sti.PathXs{i,iBlock}(j) = CurrDis*ppd(1) +  CenterX;
                % Sti.PathYs{i,iBlock}(j) = CurrDis*ppd(2) + CenterY;
                Sti.PathYs{i,iBlock}(j) = CenterY;
                Sti.PathAngs{i,iBlock}(j) = pi;
