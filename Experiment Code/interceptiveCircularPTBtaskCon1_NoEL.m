...
% For control trials, saccade to stationary targets, Xuan Jun 20
...
num_Cond = 1; % 2dir (clockwise or counterclockw) * velocity * ecc
num_multp = 40; % How many trials in each condition
...
for i = 1:num_trial
    ...
    Sti.LocXs(i) = Sti.Distance* cos(IniAngCur)*ppd + CenterX;
    Sti.LocYs(i) = Sti.Distance* sin(IniAngCur)*ppd + CenterY;
end
...
for i_trial = 1:num_trial
    ...
    if state_Sti_flag == 1
        ...
        for i_frame = 1:Sti.Frames
            ...
            Screen('DrawDots', w1, [Sti.LocXs(i_trial), Sti.LocYs(i_trial)],...
