...
% This is code is modified based on the circular motion task, used for
% doing a horizontal interceptive saccade toward horizontal moving target
...
% Xuan, Aug 11
% add random velocity. add different delay time based on the random
% velocity
...
rng('shuffle');
% inward, outward, inward, outward, inward, outward, stationary
% stationary is the last bc rem(num_Cond,num_Cond)=0
num_Cond = 7; % 2dir (inward, outward ) * 3 velocity + stationary
% % num_loc = 8; %
num_block = 4;
num_multp = 40; % How many trials in each condition, location and blocks
num_trial = num_Cond * num_multp;
num_trial_all = num_Cond * num_block * num_multp;
...
% time after stimulus osnet, FP disappear
% for         stationary,   15,     -15,    30,    -30     45     -45
FP.OffDurRange = [0,1000; 0,1000; 0,1000; 0 ,400; 0 ,400; 0,200; 0,200]; 
%FP.Preduration = 600;
...
Sti.IniAng = pi; % Horizontal left
Sti.DirMove = [1,-1]; % moving inward or o utward
Sti.Speed = [15,30,45]; %30 deg/s 
% speed * dir = vel
% positive, to right
% negative, tp left
% 0, +15, -15, +30, -30, +45, -45
Sti.Vel =        [0,     +15,    -15,    +30,   -30,     +45,   -45];
Sti.DisRange = [-18,-3; -25,20; -1,-30; -25,20; -1,-30; -25,20; -1,-30];
% Sti.DisRange1 = [-25,0]; % inward initial limit to ending limit
% Sti.DisRange2 = [0,-25]; % outward initial limit to ending limit
% Sti.DisRange3 = [-16,-5]; % Stationary
Sti.WindowSize = 16; % diameter
...
if  isempty(answer)
    ...
    % cleanup; % Abort experim ent (see cleanup function below)
    return
end
...
for i = 1:num_trial_all
    ...
    FP.OffFrames(i) = round(randi(FP.OffDurRange(rem(num_CondSec(i),num_Cond)+1,:)) /1000 / ifi);
end
...
Sti.IniDis = zeros(size((num_CondSec)));
for i = 1:num_Cond
    % moving target, 1 is stationay target
    if i ~= num_Cond
        Sti.IniDis(num_CondSec==i) = Sti.DisRange(i+1,1);
    % stationary target
    elseif i == num_Cond
        Sti.IniDis(num_CondSec==i) = randi(Sti.DisRange(1,:), sum(sum(num_CondSec == i)), 1);
    end
end
...
for iBlock = 1:num_block
    for i = 1:num_trial
        ...
        for j = 1:(FP.OffFrames(i,iBlock)+Sti.SEnFrames+Sti.SacFrames+Sti.SmPFrames)
            Sti.PathXs{i,iBlock}(j) = CurrDis*ppd(1) + CenterX;
            % Sti.PathYs{i,iBlock}(j) = CurrDis*ppd(2) + CenterY;
            Sti.PathYs{i,iBlock}(j) = CenterY; % Horizontal leftward, vertical component not moving
            Sti.PathAngs{i,iBlock}(j) = pi;
            % let it stop at the ending limit
            % inward and stationary
            if Sti.Vel(rem(num_CondSec(i,iBlock),num_Cond)+1) >=0
                if CurrDis + Sti.Vel(rem(num_CondSec(i,iBlock),num_Cond)+1)*ifi <= Sti.DisRange(2,2)
                    CurrDis = CurrDis + Sti.Vel(rem(num_CondSec(i,iBlock),num_Cond)+1)*ifi;
                elseif CurrDis + Sti.Vel(rem(num_CondSec(i,iBlock),num_Cond)+1) *ifi > Sti.DisRange(2,2)
                    CurrDis = CurrDis;
                end
            % outward
            elseif Sti.Vel(rem(num_CondSec(i,iBlock),num_Cond)+1) < 0
                if CurrDis + Sti.Vel(rem(num_CondSec(i,iBlock),num_Cond)+1)*ifi >= Sti.DisRange(2,2)
                    CurrDis = CurrDis + Sti.Vel(rem(num_CondSec(i,iBlock),num_Cond)+1)*ifi;
                elseif CurrDis + Sti.Vel(rem(num_CondSec(i,iBlock),num_Cond)+1) *ifi < Sti.DisRange(2,2)
                    CurrDis = CurrDis;
...
for i_block = 1:num_block
    ...
    for i_trial = 1:num_trial
        ...
        Eyelink('Message', ['Trial Info: TCond %d, TSpeed ',num2str(Sti.Speed)], num_CondSec(i_trial,i_block)); 
