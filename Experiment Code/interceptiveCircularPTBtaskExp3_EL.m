...
% Tried to trace eye location on the console computer, July 20
...
dataDir = '/home/mayolab/CircularMotion';
...
[screen.WidthC, screen.HeightC] = Screen('DisplaySize',screen.caliEyeScreen);
screen.WidthC = screen.WidthC/10;
screen.HeightC = screen.HeightC/10;

% for the monitor screen
[screen.WidthM, screen.HeightM] = Screen('DisplaySize',screen.monitorScreen);
screen.WidthM = screen.WidthM/10;
screen.HeightM = screen.HeightM/10;
...
% Stimulus Parameters
Sti.SaccLimT = 1500; % time limit for the first saccade
Sti.SaccEntT = 200; % saccade has to enter the target window within 200ms
...
Sti.DirMove = [-1,1]; % moving clockwise or counter clockwise
Sti.Speed = 150 * pi/180; % 200 degree/s = 0.2 degree/ms
Sti.WindowSize = 6; % diameter
...
% Priority(1);
% [screen.windowC, screen.Rect] = Screen('OpenWindow',screen.screens(1),screen.BgColor);
% w1 = screen.windowC;
...
% C means Calibration screen
% M means Monitor screen

[screen.windowC, screen.RectC] = Screen('OpenWindow',screen.caliEyeScreen,screen.BgColor);
wC = screen.windowC;
ppdC = pi * (screen.RectC(3)-screen.RectC(1)) / atan(screen.WidthC/screen.Distance/2) / 360;  % pixel per degree
screen.ppdC = ppdC;
[CenterXC, CenterYC] = RectCenter(screen.RectC);

[screen.windowM, screen.RectM] = Screen('OpenWindow',screen.monitorScreen,screen.BgColor);
wM = screen.windowM;
ppdM = pi * (screen.RectM(3)-screen.RectM(1)) / atan(screen.WidthM/screen.Distance/2) / 360;  % pixel per degree
screen.ppdM = ppdM;
[CenterXM, CenterYM] = RectCenter(screen.RectM);

% use calibration screen's frame rate
fps = Screen('FrameRate',wC);                      % frames per second, depend on monitor refresh rate
ifi = Screen('GetFlipInterval', wC);               % inter flip interval
if ifi==0
    ifi=1/fps;
end
screen.fpsC = fps;
screen.ifiC = ifi;
screen.CenterXC = CenterXC;
screen.CenterYC = CenterYC;
...
FP.sizePixelC = round(FP.Size * ppdC );
FP.LocXC = FP.Distance * cos(FP.Angle) * ppdC  + CenterXC;
FP.LocYC = FP.Distance * sin(FP.Angle) * ppdC  + CenterYC;
FP.PosC = [FP.LocXC - round(FP.sizePixelC/2),FP.LocYC - round(FP.sizePixelC/2),FP.LocXC + round(FP.sizePixelC/2),FP.LocYC + round(FP.sizePixelC/2)];
FP.WindowPixelC = round(FP.WindowSize * ppdC );
Sti.sizePixelC = round(Sti.size * ppdC );
Sti.WindowPixelC = round(Sti.WindowSize * ppdC );

% for monitor screen
FP.sizePixelM = round(FP.Size * ppdM );
FP.LocXM = FP.Distance * cos(FP.Angle) * ppdM  + CenterXM;
FP.LocYM = FP.Distance * sin(FP.Angle) * ppdM  + CenterYM;
FP.PosM = [FP.LocXM - round(FP.sizePixelM/2),FP.LocYM - round(FP.sizePixelM/2),FP.LocXM + round(FP.sizePixelM/2),FP.LocYM + round(FP.sizePixelM/2)];
FP.WindowPixelM = round(FP.WindowSize * ppdM );
Sti.sizePixelM = round(Sti.size * ppdM );
Sti.WindowPixelM = round(Sti.WindowSize * ppdM );
...
for i = 1:num_trial
    ...
    if  num_CondSec(i) == 1 % counterclockwise
        for j = 1:(FP.OffFrames(i)+Sti.SacFrames+Sti.SmPFrames)
            ...
            Sti.PathXsC(i,j) = Sti.Distance* cos(IniAngCur)*ppdC + CenterXC;
            Sti.PathYsC(i,j) = Sti.Distance* sin(IniAngCur)*ppdC + CenterYC;
...
Eyelink('Command','screen_pixel_coords = %ld %ld %ld %ld', 0, 0, screen.RectC(3)-1, screen.RectC(1)-1);
...
Eyelink('Message', 'DISPLAY_COORDS %ld %ld %ld %ld', 0, 0, screen.RectC(3)-1, screen.RectC(1)-1);
...
while 1
    ...
    Screen('DrawText',wC,text1,CenterXC - 100,CenterYC-50,[255 255 255]);
    Screen('DrawText',wC,text2,CenterXC - 500,CenterYC,[255 255 255]);
    Screen('DrawText',wC,text3,CenterXC - 700,CenterYC+50,[255 255 255]);
...
for i_trial = 1:num_trial
    ...
    if state_PreFP_flag == 1
        ...
        for i_frame = 1:Pre_FP.maxDuration_Frames
            ...
            if Eyelink('NewFloatSampleAvailable') > 0
                ...
                eyeX = sample.gx(EyeData.trial{i_trial}.eyeUsed); % [left eye gaze x, right eye gaze x] +1 as we're accessing a Matlab array
                eyeY = sample.gy(EyeData.trial{i_trial}.eyeUsed); % [left eye gaze y, right eye gaze y], Why +1?
            end

            % draw fixation point
            Screen('DrawDots', wC, [FP.LocXC, FP.LocYC], FP.sizePixelC, FP.Color, [], 2);
            Screen('DrawDots', wM, [FP.LocXM, FP.LocYM], FP.sizePixelM, FP.Color, [], 2);

            % draw eye's real time location

            % update screen, present the stimulus
            [~,stTime] = Screen('Flip', wC);
            Screen('Flip', wM);
            ...
            EyeData.trial{i_trial}.Condi{CondiNo}.frame{i_frame}.FPX = FP.LocXC;
            EyeData.trial{i_trial}.Condi{CondiNo}.frame{i_frame}.FPY = FP.LocYC;
            ...
            if sqrt(((FP.LocXC-eyeX)/ppdC).^2 + ((FP.LocYC - eyeY)/ppdC).^2)  <= FP.WindowSize / 2
    ...
    if state_FP_flag == 1
        ...
        for i_frame = 1:FP.OnFrames(i_trial)
            ...
            Eyelink('Message', 'FRAME_NUM %d, CONDI %d', i_frame, CondiNo);
            % Write message to EDF file to mark the start time of stimulus presentation
            % Eyelink('Message', 'FP_POS %.2f %.2f', FP.LocXC, FP.LocY);
    ...
    if state_Delay_flag == 1
        ...
        for i_frame = 1:FP.OffFrames(i_trial)
            ...
            %     Sti.PathXsC(i_trial,i_frame), Sti.PathYsC(i_trial,i_frame));
            % Screen('FillOval',w2 ,Sti.Color,StiRect);
            Screen('DrawDots', wC, [Sti.PathXsC(i_trial,i_frame), Sti.PathYsC(i_trial,i_frame)],...
                Sti.sizePixelC, Sti.Color, [], 2);
            Screen('DrawDots', wM, [Sti.PathXsM(i_trial,i_frame), Sti.PathYsM(i_trial,i_frame)],...
                Sti.sizePixelM, Sti.Color, [], 2);
            ...
            % Write message to EDF file to mark the start time of stimulus presentation
            % Eyelink('Message', 'FP_POS %.2f %.2f', FP.LocXC, FP.LocY);
            % Eyelink('Message', 'Targ_POS %.2f %.2f', Sti.PathXsC(i_trial,i_frame), Sti.PathYsC(i_trial,i_frame));
            ...
            EyeData.trial{i_trial}.Condi{CondiNo}.frame{i_frame}.FPX = FP.LocXC;
            EyeData.trial{i_trial}.Condi{CondiNo}.frame{i_frame}.FPY = FP.LocYC;
            EyeData.trial{i_trial}.Condi{CondiNo}.frame{i_frame}.targX = Sti.PathXsC(i_trial,i_frame);
            EyeData.trial{i_trial}.Condi{CondiNo}.frame{i_frame}.targY = Sti.PathYsC(i_trial,i_frame);
            EyeData.trial{i_trial}.Condi{CondiNo}.string = '';
    ...
    if state_SacResp_flag == 1
        ...
        for i_frame = 1: Sti.SacFrames
            ...
            % draw stimulus
            Screen('DrawDots', wC, [Sti.PathXsC(i_trial,i_frame1), Sti.PathYsC(i_trial,i_frame1)],...
                Sti.sizePixelC, Sti.Color, [], 2);
            Screen('DrawDots', wM, [Sti.PathXsM(i_trial,i_frame1), Sti.PathYsM(i_trial,i_frame1)],...
                Sti.sizePixelM, Sti.Color, [], 2);
            ...
            EyeData.trial{i_trial}.Condi{CondiNo}.frame{i_frame}.eyeX = eyeX;
            EyeData.trial{i_trial}.Condi{CondiNo}.frame{i_frame}.eyeY = eyeY;
            EyeData.trial{i_trial}.Condi{CondiNo}.frame{i_frame}.targX = Sti.PathXsC(i_trial,i_frame1);
            EyeData.trial{i_trial}.Condi{CondiNo}.frame{i_frame}.targY = Sti.PathYsC(i_trial,i_frame1);
            EyeData.trial{i_trial}.Condi{CondiNo}.string = '';
            ...
            if sqrt(((FP.LocXC-eyeX)/ppdC).^2 + ((FP.LocYC - eyeY)/ppdC).^2)  > FP.WindowSize / 2
                ...
                % within required time: 200 ms?
                if sqrt(((Sti.PathXsC(i_trial,i_frame1)-eyeX)/ppdC).^2 + ((Sti.PathYsC(i_trial,i_frame1) - eyeY)/ppdC).^2)  <= Sti.WindowSize / 2
                ...
                % Fixation point did not move in within the required timei
    ...
    %% clear and wait ITI
    % Clear the screen
    Screen('FillRect', wC, screen.BgColor);
    Screen('Flip', wC);
