...
%           Now go back to before
% 05-12-25: add photodiode redtection for rig 1
% 05-13-25: try to reduce time of processing
% 06-18-25: remove the first shift time when calculate the target real flip
%           first shift time is the time between PTB and trellis
%           Both graphic flip time and target time add the shift time... so
%           maybe no effect

%% run some modules or not
isReDetect = 1; % re detect the photodiode
...
%% Threshold -based lowpass filter, redetection of photodiode
phThres = 150; % amplitude threshold is set to 120
Fs = 1000; % Sampling frequency
Fc = 30; % Cutoff frequency Hz
[b, a] = butter(4, Fc/(Fs/2), 'low'); % 4th-order low-pass Butterworth

for iTrial = 1:size(Datax,1)
    original_signal = TlisD.photodiode{iTrial};
    % pass the amplitdue first
    mask = original_signal > phThres;
    % Keep only the bursts
    burst_signal = original_signal .* mask;

    % apply low pass filter
    photodiode_filtered=filtfilt(b,a,burst_signal);
    photodiode_bi=photodiode_filtered>50; %to bi data, take >10 photodiode

    % detect photodiode event
    zero2one = find(diff(photodiode_bi) == 1); % find 0 to 1
    one2zero = find(diff(photodiode_bi) == -1);% find 1 to 0
    photodiode_event = sort([zero2one,one2zero]);

    % if the signal start at the phase 0 of a cycle, the photodiode_bi will
    % have a quick flip at the beginning. solve this issue
    if ~isempty(photodiode_event) && photodiode_event(1)<=5
        photodiode_event(1) = [];
        photodiode_bi(1:5) = 1;
    end
    %
    % put the result into TlisD
    TlisD.photodiode_bi1{iTrial} = TlisD.photodiode_bi{iTrial};
    TlisD.photodiode_event1{iTrial} = TlisD.photodiode_event{iTrial};
    TlisD.photodiode_bi{iTrial} = photodiode_bi;
    TlisD.photodiode_event{iTrial} = photodiode_event;
end

% plot(1:length(original_signal),original_signal)
% hold on
% plot(1:length(burst_signal),burst_signal)
% plot(1:length(photodiode_filtered),photodiode_filtered,'Linewidth',1.5)
% plot(1:length(photodiode_bi),photodiode_bi*1000,'Linewidth',1.5)
...
for iTrial = 1:size(Datax,1)
    ...
    if iscell(PsyTBD.data)
        PsyTBDdataAll{iTrial} = PsyTBD.data{iTrial};
    else
        PsyTBDdataAll{iTrial} = PsyTBD.data(iTrial);
    end
end
...
% for iTrial = 574
    % write the psyTB photodiode time
    Datax.stateReal{iTrial} = PsyTBDdataAll{iTrial}.DiodeFlipStates;
    Datax.pddPsyTB{iTrial} = PsyTBDdataAll{iTrial}.photodiode;
    ...
    for iState = 1:length(curstate)
        if iscell(Datax.trialST)
            curstateTime(iState) = curstate{iState}.time(1) - Datax.trialST{iTrial};
        else
            curstateTime(iState) = curstate{iState}.time(1) - Datax.trialST(iTrial);
        end
    end
    ...
    if isempty(stateReal)
        % sometimes the photodiode info are recorded in the next trial
        stateRealNext = PsyTBDdataAll{iTrial+1}.DiodeFlipStates;
        if sum(strcmp(stateRealNext, 'start_autogenerated')) == 2 % check if its really duplicated
            EndIdx = find(strcmp(stateRealNext, 'start_autogenerated'),1,'last')-1;
            % for the current trial
            Datax.stateReal{iTrial} = stateRealNext(1:EndIdx);
            stateReal = Datax.stateReal{iTrial};
            % for the next trial
            PsyTBDdataAll{iTrial+1}.DiodeFlipStates(1:EndIdx) = [];
        else
            % record this error, means this trial even didn't start.. wired
    ...
    if length(stateReal)< length(pddTlisD)
        ...
        % pddTlisD(1) = [];
        % it may also due to the reason that within 20ms there are wrong detect data
    end
    ...
    % have to make sure that pddTlisD == stateReal, if not record it
    if length(pddTlisD) == length(stateReal)
...
for iTrial = 1:size(Datax,1)
    ...
    if any(contains(stateSent,'T1_reach'))
        ...
        elseif contains(Datax.trialType{iTrial},'Sta')
            ...
            if sum(contains(stateReal,'T0_hold')) == 0 || sum(contains(stateReal,'T1_reach_state_moving')) == 0
                Datax.trialErr(iTrial) = 2;
                continue
            end
            % check the FixOn Event, T0_reach may combined with T0_hold
            ...
            % Removed RT state, Now there is T1_reach or T1_reachin
            % this is also target on time
            FixItvENum = find(contains(stateReal,'delay'));
            if isempty(FixItvENum)
                % could be T1_reach_state
                FixItvENum = find(contains(stateReal,'T1_reach_state_moving'));
            end
            % check the Go cue num
            GoCueNum = find(contains(stateReal,'T1_reach_state_moving'));
        end
    else
...
for iTrial = 1:size(Datax,1)
    ...
    % record the graphic flip time
    % remove the shift time here
    GrapFlipTime = [];
    if ~isempty(PsyTBDdataAll{iTrial}.graphics_fliptimes)
        if isstruct(PsyTBDdataAll{iTrial}.graphics_fliptimes)
            if ischar(PsyTBDdataAll{iTrial}.graphics_fliptimes.fliptimes)
                GrapFlipTime = (str2num(PsyTBDdataAll{iTrial}.graphics_fliptimes.fliptimes)'- TrialST).*1000;
            else
                GrapFlipTime = (PsyTBDdataAll{iTrial}.graphics_fliptimes.fliptimes'- TrialST).*1000;
            end
        else
            GrapFlipTime = (PsyTBDdataAll{iTrial}.graphics_fliptimes(1,:)'- TrialST).*1000;
        end
    end
    Datax.GFlipTime{iTrial} = GrapFlipTime;
    % continue if no GrapFlipTime
    if isempty(Datax.GFlipTime{iTrial})
        if Datax.trialErr(iTrial) ~=0
            Datax.trialErr(iTrial) = 3; % no graphic flip time error
        end
        continue
    end
    ...
    for iNames = 1:length(TargetNames)
        ...
        Datax.(['T',mat2str(iNames-1),'Time']){iTrial} = ...
            (PsyTBD.targets{iTrial}.(TargetNames{iNames}).timestamp' - TrialST).*1000;
...
for iTrial = 1:size(Datax,1)
    if iscell(PsyTBD.System_Properties)
    ...
    Datax.EyeLocXYPTB{iTrial} = PsyTBDdataAll{iTrial}.eyepos';
    ...
    Datax.EyeLocXYTlis{iTrial} = pix2deg([EyeLocXTlisTemp; EyeLocYTlisTemp]','cart',e.LastParameters.screenparams)';
    ...
    % this only works for linear speed
    Datax.EyeVelMark{iTrial} = abs(Datax.EyeVel{iTrial}) >=  VelThrs;
