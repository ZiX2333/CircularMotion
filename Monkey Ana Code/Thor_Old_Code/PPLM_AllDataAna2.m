...
if ismember(SecNum,SecPlots)
    ...
    SaveName = [];
    SaveName = '/SacEndErr_Tar_2E_XY_Normed_AllSessi';
    ...
    for iCond = CondI
        ...
        for iSession = 1:SessiSize
            ...
            Datax1 = []; sbd = [];
            Datax1 = DataxAll(iSession).Datax1;
            sbd = sbdAll(iSession).sbd;
...
%% 2 plot the linear regression of all the subjects' RT result ZS
SecNum = SecNum+1;
if ismember(SecNum,SecPlots)
    iFigAcc = iFigAcc+1;
    figure(iFigAcc)
    set(gcf,'Position',[31,228,1486,712]);
    SaveName = [];
    SaveName = '/SacEndErr_RT_Normed_AllSessi';
    ...
    for iCond = CondI
        ...
        for iSession = 1:SessiSize
            ...
            CurXV = sbd.SacRTGoc1Norm(datas1)';
            CurYV = rad2deg(sbd.SacEndErrAng2ESignDeSta(datas1))';
            
            FM_CartScat(CurXV, CurYV, iCond, XLim, YLim);
        ...
        [StatResult] = FM_FitLinearR(XVAll,YVAll);
        % plot the linear regression results
        XVLim = [min(XVAll),max(XVAll)]; YVLim = [min(YVAll),max(YVAll)];
        ...
        text(XLim(2)-0.5, YLim(2)-(YLim(2)-YLim(1))/10, value_text, 'FontSize', 14);
        hold off

        if iCond == 1
            xlabel('Normalized RT'); ylabel('Ending Error, deg');
        end
        hold off
        set(gca,'FontSize',15)
    end
...
%% 3 plot the linear regression of all the subjects' SmP result ZS [50,150]
SecNum = SecNum+1;
if ismember(SecNum,SecPlots)
    iFigAcc = iFigAcc+1;
    figure(iFigAcc)
    set(gcf,'Position',[31,228,1486,712]);
    SaveName = [];
    SaveName = '/SacEndErr_SmPL_Normed_50_150_AllSessi';
    % MarkType = {'o','+','*','.','x','|','square','diamond','^','pentagram','hexagram'};
    XLim = [-0.1,1.1];
    YLim = [-70,70];
    for iCond = CondI
        ...
        for iSession = 1:SessiSize
            ...
            CurXV = sbd.SmPLVelGoc1Norm(datas1,1)';
            CurYV = rad2deg(sbd.SacEndErrAng2ESignDeSta(datas1))';
        ...
        if iCond == 1
            xlabel('Normalized Pursuit Vel [50,150]ms'); ylabel('Ending Error, deg');
        end
        hold off
        set(gca,'FontSize',15)
    end
...
%% 4 plot the linear regression of all the subjects' SmP result ZS [80,180]
SecNum = SecNum+1;
if ismember(SecNum,SecPlots)
    iFigAcc = iFigAcc+1;
    figure(iFigAcc)
    set(gcf,'Position',[31,228,1486,712]);
    SaveName = [];
    SaveName = '/SacEndErr_SmPL_Normed_80_180_AllSessi';
    % MarkType = {'o','+','*','.','x','|','square','diamond','^','pentagram','hexagram'};
    XLim = [-0.1,1.1];
    YLim = [-70,70];
    for iCond = CondI
        ...
        for iSession = 1:SessiSize
            ...
            CurXV = sbd.SmPLVelGoc1Norm(datas1,2)';
            CurYV = rad2deg(sbd.SacEndErrAng2ESignDeSta(datas1))';
        ...
        if iCond == 1
            xlabel('Normalized Pursuit Vel [80,180]ms'); ylabel('Ending Error, deg');
        end
        hold off
        set(gca,'FontSize',15)
    end
...
%% 5 plot the linear regression of all the subjects' SmP result ZS [100,200]
SecNum = SecNum+1;
if ismember(SecNum,SecPlots)
    iFigAcc = iFigAcc+1;
    figure(iFigAcc)
    set(gcf,'Position',[31,228,1486,712]);
    SaveName = [];
    SaveName = '/SacEndErr_SmPL_Normed_100_200_AllSessi';
    % MarkType = {'o','+','*','.','x','|','square','diamond','^','pentagram','hexagram'};
    XLim = [-0.1,1.1];
    YLim = [-70,70];
    for iCond = CondI
        ...
        for iSession = 1:SessiSize
            ...
            CurXV = sbd.SmPLVelGoc1Norm(datas1,3)';
            CurYV = rad2deg(sbd.SacEndErrAng2ESignDeSta(datas1))';
        ...
        if iCond == 1
            xlabel('Normalized Pursuit Vel [100,200]ms'); ylabel('Ending Error, deg');
        end
        hold off
        set(gca,'FontSize',15)
    end
...
if ismember(SecNum,SecPlots)
    ...
    for iCond = CondI
        ...
        for iSession = 1:SessiSize
            klDivg(iSession,:) = DataxAll(iSession).klDivg(iCond,:);
            polarplot(vfPDFSamples,klDivg(iSession,:),'LineWidth',1.5,'Color',colorRGB1(iCond,:));
            hold on
        end
        ...
        klDivgAllSE(iCond,:) = std(klDivg)/SessiSize;
