...
for iTrial = 1:size(Datax,1)
    ...
    if ~isempty(PsyTBDdataAll{iTrial}.graphics_fliptimes)
        if isstruct(PsyTBDdataAll{iTrial}.graphics_fliptimes)
            if ischar(PsyTBDdataAll{iTrial}.graphics_fliptimes.fliptimes)
                GrapFlipTime = (str2num(PsyTBDdataAll{iTrial}.graphics_fliptimes.fliptimes)'- TrialST).*1000 + shiftTime;
            else
                GrapFlipTime = (PsyTBDdataAll{iTrial}.graphics_fliptimes.fliptimes'- TrialST).*1000 + shiftTime;
            end
        else
            GrapFlipTime = (PsyTBDdataAll{iTrial}.graphics_fliptimes(1,:)'- TrialST).*1000 + shiftTime;
        end
    end
    ...
    for iNames = 1:length(TargetNames)
        ...
        if isempty(Datax.(['T',mat2str(iNames-1),'Time']){iTrial})
            ...
            if isempty(GrapFlipTime)
                Datax.trialErr(iTrial) = 2; %have T1Time but no Graphic flip time
                continue
            end
