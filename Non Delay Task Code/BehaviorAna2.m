...
for iTrial = 1:size(DatafN,2)
    ...
    if sbd.SacETmGoc1(iTrial)+500 <= length(DatafN(iTrial).EyeLocRVel(6,:))
        ...
        sbd.SmPLVelAllGoc1(iTrial,1:length(DatafN(iTrial).EyeLocRVel(6,:))-(sbd.SacETmGoc1(iTrial))) = ...
...
for iCond = CondI
    ...
    if iCond <5
        XNorm = []; YNorm = []; XIndAll = [];
        [~, YNorm, ~] = F_CartScaMovNorm(winSize,stepSize,XV',XVbase',YV',YVbase',[0,deg2rad(315)]);
    else
        XNorm = []; YNorm = []; XIndAll = [];
        [~, YNorm, ~] = F_CartScaMovNorm(winSize,stepSize,XV',XVbase',YV',-YVbase',[0,deg2rad(315)]);
    end
...
for iCond = CondI
    ...
    % do the normalization
    XNorm = []; YNorm = []; XIndAll = [];
    [~, YNorm, ~] = F_CartScaMovNorm1(winSize,stepSize,XV',XVbase',YV',YVbase',[0,deg2rad(315)]);
