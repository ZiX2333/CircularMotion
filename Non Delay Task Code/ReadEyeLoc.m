...
% Feb 23 2024, important adjustment on the order of theta and R (it used to be R and theta)
% Feb 26 2024, adjust on data time window
% Mar 19 2025, adjust the accmulation distance that start at TimeFixOn2 (subject look at go cue) 
% to avoid the effect of eye start at a too large location
...
for iTrial = 1:size(Dataf,2)
    ...
    EyeLocDispAcc = zeros(size(EyeLocDisp));
    % EyeLocDispAcc = cumsum(EyeLocDisp);
    % change here!! to make the very initial eye location doesn't affect accumulation
    if ~isnan(Dataf(iTrial).TimeFixOn2)
        EyeLocDispAcc(Dataf(iTrial).TimeFixOn2:end) = cumsum(EyeLocDisp(Dataf(iTrial).TimeFixOn2:end));
    end

    % X, Y, Theta, Radius, Displacement, Accumulate Displacement
    Dataf(iTrial).EyeLocR = [EyeLocRX;EyeLocRY;EyeLocRAng;EyeLocRRds;EyeLocDisp;EyeLocDispAcc];
    ...
    % X, Y, Radius velocity
    Dataf(iTrial).EyeLocRVel = [EyeLocVelRX;EyeLocVelRY;EyeLocVelRAng;EyeLocVelRRds;EyeLocVelDisp;EyeLocVelDispAcc];
    ...
    Dataf(iTrial).EyeLocRAcc = [EyeLocAccRX;EyeLocAccRY;EyeLocAccRAng;EyeLocAccRRds;EyeLocAccDisp;EyeLocAccDispAcc];
    ...
    elseif Dataf(iTrial).TrialStatus == 1 || Dataf(iTrial).TrialStatus == 5
        ...
        try
            ...
            Dataf(iTrial).EyeLocRGoc = [Dataf(iTrial).EyeLocR(:,FixT+FixW(1):end);...
                Dataf(iTrial).EyeLocRVel(:,FixT+FixW(1):end);...
                Dataf(iTrial).EyeLocRVelM(:,FixT+FixW(1):end);...
                Dataf(iTrial).EyeLocRAcc(:,FixT+FixW(1):end)];
        end
        ...
        try
            ...
            Dataf(iTrial).EyeLocRGoc = [Dataf(iTrial).EyeLocR(:,TarT+TarW(1):end);...
                Dataf(iTrial).EyeLocRVel(:,TarT+TarW(1):end);...
                Dataf(iTrial).EyeLocRVelM(:,TarT+TarW(1):end);...
                Dataf(iTrial).EyeLocRAcc(:,TarT+TarW(1):end)];
        end
        ...
        try
            ...
            Dataf(iTrial).EyeLocRGoc = [Dataf(iTrial).EyeLocR(:,GocT+GocW(1):end);...
                Dataf(iTrial).EyeLocRVel(:,GocT+GocW(1):end);...
                Dataf(iTrial).EyeLocRVelM(:,GocT+GocW(1):end);...
                Dataf(iTrial).EyeLocRAcc(:,GocT+GocW(1):end)];
        end

        % Relative to Saccade Onset Time to the end of Trial: [-500ms, end]
        Dataf(iTrial).EyeLocRSac = nan;

        % Relative to Gocue Onset Time to the end of Trial: [-500ms, end]
...
for iTrial = 1:size(Dataf,2)
    ...
    % fill in the eye location trace related to saccade onset:
    if isempty(Dataf(iTrial).SacTimeGoc2)
        continue
    end
    SacT = Dataf(iTrial).SacTimeGoc2(1,1);
    Dataf(iTrial).EyeLocRSac = [Dataf(iTrial).EyeLocR(:,SacT+SacW(1):end);...
            Dataf(iTrial).EyeLocRVel(:,SacT+SacW(1):end);...
            Dataf(iTrial).EyeLocRVelM(:,SacT+SacW(1):end);...
            Dataf(iTrial).EyeLocRAcc(:,SacT+SacW(1):end)];
end
...
if plotAllfig == 1
    ...
    % for iTrial  = find([Dataf.DurDelay] == 0)
    for iTrial  = 1:size(Dataf,2)
        if Dataf(iTrial).TrialStatus ~= 1 && Dataf(iTrial).TrialStatus ~= 5
            continue
        end
        ...
        p3 = plot(GocW(1):length(EyeLoc(1,:))+GocW(1)-1,EyeLoc(10,:)/100,'LineWidth',2,'Color',colorRGB(3,:));
        ...
        plot(TimeV1,EyeLoc(10,TimeV1-GocW(1))/100,'ko','MarkerSize',10);
        ...
        if ~isempty(TimeS2)
            ...
            plot(TimeV2,EyeLoc(10,TimeV2-GocW(1))/100,'ro','MarkerSize',10);
        end
        ...
        p3 = polarplot(wrapToPi(EyeLoc(3,:)),EyeLoc(4,:),'LineWidth',2,'Color',[0.4660 0.6740 0.1880]);
        hold on
        ...
        polarplot(EyeLoc(3,TimeS1-GocW(1)),EyeLoc(4,TimeS1-GocW(1)),'kx', 'MarkerSize', 8,'LineWidth',1.5)
        polarplot(EyeLoc(3,TimeE1-GocW(1)),EyeLoc(4,TimeE1-GocW(1)),'kx', 'MarkerSize', 8,'LineWidth',1.5)
        polarplot(EyeLoc(3,TimeV1-GocW(1)),EyeLoc(4,TimeV1-GocW(1)),'ko', 'MarkerSize', 8,'LineWidth',1.5)

        % mark the second saccade onset and offset point and peak velocity location
        if ~isempty(TimeS2)
            polarplot(EyeLoc(3,TimeS2-GocW(1)),EyeLoc(4,TimeS2-GocW(1)),'rx', 'MarkerSize', 8,'LineWidth',1.5)
            polarplot(EyeLoc(3,TimeE2-GocW(1)),EyeLoc(4,TimeE2-GocW(1)),'rx', 'MarkerSize', 8,'LineWidth',1.5)
            polarplot(EyeLoc(3,TimeV2-GocW(1)),EyeLoc(4,TimeV2-GocW(1)),'ro', 'MarkerSize', 8,'LineWidth',1.5)
        end
