...
% adjusted on Sep 04 23, add real time target location check.

% adjusted on Jan 01 24, add a ini time check for the first trial due to
% the possbile disalign between the real data and recording data

% trial_NumAll = size(EyeData.trial,1) * size(EyeData.trial,2);

% Adjusted on Feb 25, sttime0 detect 'STARTSAMPLES' not '!MODE'

% Adjusted on Feb 26, a big change on Error detection: trials with zero
% delays will not have state 3, which was count as error before but
% actually not, need to be detect specifically

% Adjusted on Feb 29,

% Adjusted on Feb 03 25, add if condition if eyelink is recorded in 1k

%% check if recorded in 1k
if is1k == 1
    edfStruct.FSAMPLE.time = repelem(edfStruct.FSAMPLE.time,2);
...
for iEvent = 1:size(edfStruct.FEVENT,2)
    ...
    if ReadFlag == 1
        ...
        if ~isempty(strfind(edfStruct.FEVENT(iEvent).codestring, 'STARTSAMPLES'))
...
for iTrial = 1:length(DataEdf)
    ...
    % Check if the true start recording time is later than recorded
    % usually happens in the first trial, the real recording time may later
    % than recorded time. Adjust to the real recording time if the end time
    % exist
    if iTrial == 1 && TimeSeq(2) < edfStruct.FSAMPLE.time(1) && TimeSeq(10) > edfStruct.FSAMPLE.time(1)
        TimeSeq(2) = edfStruct.FSAMPLE.time(1);
    end
    ...
    % TrialStatus = 3, error before stimulus onset/ or after
    % TrialStatus = 4, error before gocue onset/ or after
    % TrialStatus = 5, error before smooth pursuit/ or after?
    ...
    if DataEdf(iTrial).TrialStatus == 0
        ...
        if isnan(DataEdf(iTrial).sttime3)
            Dataf(iTrial).TrialStatus = 1; % No error! just zero delay
            DataEdf(iTrial).sttime3 = DataEdf(iTrial).sttime4;
            TimeSeq(5) = TimeSeq(6);
        elseif isnan(DataEdf(iTrial).sttime5)
            Dataf(iTrial).TrialStatus = 5; % error at smooth pursuit
        elseif sum(isnan(TimeSeq(1:length(TimeSeq)-3))) == 0
            Dataf(iTrial).TrialStatus = 1; % No error!
        end
        % besides these possibilities, errors are really hard to explain
    ...
    for iEvent = 1:size(DataEdf(iTrial).AllEvent,2)
        ...
        if ~isempty(strfind(DataEdf(iTrial).AllEvent(iEvent).message, 'Targ_POS'))
            ...
            iTime = double(DataEdf(iTrial).AllEvent(iEvent).sttime-Dataf(iTrial).TimeSeq(2));
