...
% Adjusted on Apr 9, RT de stationary
%% Txt information
...
%% 1 RT and Saccade Ending Error and Target Location, All DeSta
SecNum = SecNum+1;
if ismember(SecNum,SecPlots)
    ...
    SaveName = [];
    SaveName = '/RT_TargSacEnd_SacEndErr2E_DeSta';
    ...
    for iCond = CondI
        ...
        YV1 = sbd.SacEndErrAng2ESignDeSta(datas1);
        ...
        if iCond == 1
            ylabel('RT-Stationry, ms')
        end

        % plot the sliding window on average
        XAve = []; YAve = []; YStd = [];
...
%% 2, RT and Ending error, after de stationary
SecNum = SecNum+1;
if ismember(SecNum,SecPlots)
    iFigAcc = iFigAcc+1;
    figure(iFigAcc)
...
%% 3 Smooth Pursuit Velocity as a function of time seperated by ending error
SecNum = SecNum+1;
if ismember(SecNum,SecPlots)
    iFigAcc = iFigAcc+1;
    figure(iFigAcc)
...
%% 4 lower Q and higher Q distributions eye ending location
SecNum = SecNum+1;
if ismember(SecNum,SecPlots)
    iFigAcc = iFigAcc+1;
    figure(iFigAcc)
...
%% 5 lower Q and higher Q distributions eye ending location
SecNum = SecNum+1;
if ismember(SecNum,SecPlots)
    iFigAcc = iFigAcc+1;
    figure(iFigAcc)
