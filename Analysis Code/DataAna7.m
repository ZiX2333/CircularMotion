    ...
    for iSubJ = 1:4
        ...
        for iCond = CondI
            ...
            for iTrial = datas
                ...
                TarLocAng(iTrialI) = Dataf1(iTrial).SacLocGoc2{1}(4,end);
                if TarLocAng(iTrialI) > pi/2 && TarLocAng(iTrialI) < pi
...
for iCond = CondI
    ...
    for iTrial = datas
        iTrialI = iTrialI+1;
        TarLocAng(iTrialI) = wrapToPi(Dataf1(iTrial).SacTarGoc1(iFig+2,3));
        if TarLocAng(iTrialI) > pi/2 && TarLocAng(iTrialI) < 3*pi/2
            TarLocAng(iTrialI) = TarLocAng(iTrialI) - 2*pi;
        end
...
SaveName{1} = '/SacCur_TarLocSacOn_Saccter_LinearFit';
SaveName{2} = '/SacCur_TarLocSacOff_Saccter_LinearFit';
...
Fact21 = [-3*pi/2,  -5*pi/4,-pi,    -3*pi/4;...
          pi/4,  0,    -pi/4,-pi/2]; % target location
Fact22 = [-5*pi/4,-pi,    -3*pi/4,-pi/2;...
          pi/2,  pi/4,  0,    -pi/4]; % target location
...
DataGroup = table(SubjIDVal', Fact2Val', Fact4Val', OutCome', ...
    'VariableNames', {'Subject', 'TargetLocation', 'Direction', 'OutCome'});
lme = [];
lme = fitlme(DataGroup, 'OutCome ~ TargetLocation*Direction + (1|Subject)');
disp(lme.Coefficients);
anovaResults = anova(lme);
disp(anovaResults);