...
%% 1 plot the eye traces 2E with the link between, color code the link
% SecNum = SecNum+1;
% if ismember(SecNum,SecPlots)
    iFigAcc = iFigAcc+1;
    figure(iFigAcc)
    set(gcf,'Position',[31,228,1486,712]);
    SaveName = [];
    SaveName = '/RawEyeTrc2E_TarSacEnd_ColoredLinked';
    t = tiledlayout('flow');
    for iCond = CondI
        nexttile
        if iCond == 5
        ...
        for iTrial = datas1
            ...
            % TimeE = Dataf1(iTrial).SacTimeGoc1(2)-Dataf1(iTrial).TimeGocOn +GocC;
            % Polar Plot
            p1 = polarplot(EyeLoc(3,:),EyeLoc(4,:),'LineWidth',1,'Color',[colorRGB(iCond,:),0.6]);
            TarLocTemp(1) = sbd.TarEnd2E(3,iTrial);
            TarLocTemp(2) = sbd.TarEnd2E(4,iTrial);
            hold on
            s1 = polarscatter(TarLocTemp(1),TarLocTemp(2),10,'black','filled');
            % add lines betwee point to the end
            if sbd.SacEndErrAng2ESignDeSta(iTrial) >=0
                polarplot([EyeLoc(3,end),TarLocTemp(1)],[EyeLoc(4,end),TarLocTemp(2)],...
                    'LineWidth',1,'Color','k','LineStyle','-');
            elseif sbd.SacEndErrAng2ESignDeSta(iTrial) <0
                polarplot([EyeLoc(3,end),TarLocTemp(1)],[EyeLoc(4,end),TarLocTemp(2)],...
                    'LineWidth',1.5,'Color','m','LineStyle','-');
            end
        end
        rlim([0,12])
        title(LegText{iCond},'FontWeight','normal');
...
%% 2 check if the ending error versus targ dir is correct by the new method
% SecNum = SecNum+1;
% if ismember(SecNum,SecPlots)
    iFigAcc = iFigAcc+1;
    figure(iFigAcc)
    set(gcf,'Position',[31,228,1486,712]);
    SaveName = [];
    SaveName = '/SacEndErr_Tar_2E_XY_NoDeSta';
    ...
    for iCond = CondI
        ...
        if iCond == 1
            ...
            ylabel('Sacc Targ Direction Difference, no -sta, deg')
        end
        hold on
...
% if ismember(SecNum,SecPlots)
    ...
    set(gcf,'Position',[31,228,1486,712]);
    SaveName = [];
    SaveName = '/SacEndErr_Tar_2E_XY_CondDiff';
    ...
    for iCond = CondI
        ...
        if iCond == 1
            ...
            ylabel('Sacc Targ Direction Difference no -sta, deg')
        elseif iCond == 2
            ylabel('Sacc Targ Direction Difference -sta, deg')
        end
        hold on
        xline(180,'LineWidth',1.5,'LineStyle','--');
        xline(90,'LineWidth',1.5,'LineStyle','--');
...
%% 4 plot the KL divergence across conditions
SecNum = SecNum+1;
if ismember(SecNum,SecPlots)
    iFigAcc = iFigAcc+1;
    figure(iFigAcc)
...
%% 5 RT and ending error relationship
SecNum = SecNum+1;
if ismember(SecNum,SecPlots)
    iFigAcc = iFigAcc+1;
    figure(iFigAcc)
...
%% 7 lower Q and higher Q distributions eye ending location
% SecNum = SecNum+1;
% if ismember(SecNum,SecPlots)
    iFigAcc = iFigAcc+1;
    figure(iFigAcc)
    set(gcf,'Position',[31,228,1486,712]);
    SaveName = [];
    SaveName = '/SacEnd2E_HighLowerQDist';
    ...
    for iCond = CondI
        ...
        datas1 = find([Dataf1.TarDir1] == iCond & ([Dataf1.TrialStatus] == 1 | [Dataf1.TrialStatus] == 5));
        EndErr = []; SaccLocT = []; SaccLocR = [];

        EndErr = sbd.SacEndErrAng2ESignDeSta(datas1);
        SaccLocT = wrapTo2Pi(sbd.SacEnd2E(3,datas1));
        SaccLocR = sbd.SacEnd2E(4,datas1);

        hl = []; hp = [];
        % lower Q
        polarscatter(SaccLocT(GrpIdx{iCond}{1}),SaccLocR(GrpIdx{iCond}{1}),100,'Marker','.','MarkerEdgeColor',colorRGB(iCond,:),'LineWidth',1.5);
        hold on
        % KS density
        vfEstimate = circ_ksdensity(SaccLocT(GrpIdx{iCond}{1}), vfPDFSamples, [0, 2*pi], fSigma);
        polarplot(vfPDFSamples,vfEstimate*10,'Color',colorRGB2(iCond,:),'LineWidth',1.5,'LineStyle','-')

        % higher Q
        polarscatter(SaccLocT(GrpIdx{iCond}{4}),SaccLocR(GrpIdx{iCond}{4}),100,'Marker','x','MarkerEdgeColor',colorRGB(iCond,:),'LineWidth',1.5);
        % KS density
        vfEstimate = circ_ksdensity(SaccLocT(GrpIdx{iCond}{4}), vfPDFSamples, [0, 2*pi], fSigma);
        polarplot(vfPDFSamples,vfEstimate*10,'Color',colorRGB2(iCond,:),'LineWidth',1.5,'LineStyle','--')
...
%% 8 lower Q and higher Q distributions eye ending location
SecNum = SecNum+1;
if ismember(SecNum,SecPlots)
    iFigAcc = iFigAcc+1;
    figure(iFigAcc)
    set(gcf,'Position',[31,228,1486,712]);
    SaveName = [];
    SaveName = '/TargEnd2E_HighLowerQDist';
    ...
    for iCond = CondI
        ...
        datas1 = find([Dataf1.TarDir1] == iCond & ([Dataf1.TrialStatus] == 1 | [Dataf1.TrialStatus] == 5));
        EndErr = []; TargLocT = []; TargLocR = [];

        EndErr = sbd.SacEndErrAng2ESignDeSta(datas1);
        TargLocT = wrapTo2Pi(sbd.TarEnd2E(3,datas1));
        TargLocR = sbd.TarEnd2E(4,datas1);

        hl = []; hp = [];
        % lower Q
        polarscatter(TargLocT(GrpIdx{iCond}{1}),TargLocR(GrpIdx{iCond}{1}),100,'Marker','.','MarkerEdgeColor',colorRGB(iCond,:),'LineWidth',1.5);
        hold on
        % KS density
        vfEstimate = circ_ksdensity(TargLocT(GrpIdx{iCond}{1}), vfPDFSamples, [0, 2*pi], fSigma);
        polarplot(vfPDFSamples,vfEstimate*10,'Color',colorRGB2(iCond,:),'LineWidth',1.5,'LineStyle','-')

        % higher Q
        polarscatter(TargLocT(GrpIdx{iCond}{4}),TargLocR(GrpIdx{iCond}{4}),100,'Marker','x','MarkerEdgeColor',colorRGB(iCond,:),'LineWidth',1.5);
        % KS density
        vfEstimate = circ_ksdensity(TargLocT(GrpIdx{iCond}{4}), vfPDFSamples, [0, 2*pi], fSigma);
        polarplot(vfPDFSamples,vfEstimate*10,'Color',colorRGB2(iCond,:),'LineWidth',1.5,'LineStyle','--')
