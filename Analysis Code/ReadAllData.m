...
for iTrial = 1:length(DataEdf)
    ...
    % rem = 2 clockwise
    % rem = 0 control
    Dataf(iTrial).TarDir = rem(num_CondSec(iTrial),3);
    ...
    % Target Path
    Dataf(iTrial).TarPathX = zeros(2,length(Sti.PathXs{iTrial}));
    Dataf(iTrial).TarPathY = zeros(2,length(Sti.PathYs{iTrial}));
    Dataf(iTrial).TarPathAng = zeros(2,length(Sti.PathAngs{iTrial}));

    Dataf(iTrial).TarPathX(1,:) = Sti.PathXs{iTrial};
    Dataf(iTrial).TarPathY(1,:) = Sti.PathYs{iTrial};
    Dataf(iTrial).TarPathAng(1,:) = Sti.PathAngs{iTrial};

    iFrame = [];
    for iEvent = 1:length(DataEdf(iTrial).AllEvent)
        if ~isempty(strfind(DataEdf(iTrial).AllEvent(iEvent).message, 'StateI 3'))
            iFrame = [iFrame,DataEdf(iTrial).AllEvent(iEvent).sttime];
    ...
    Dataf(iTrial).TarPathX(2,1:length(iFrame)) = iFrame-Dataf(iTrial).TimeSeq(2);
    Dataf(iTrial).TarPathY(2,1:length(iFrame)) = iFrame-Dataf(iTrial).TimeSeq(2);
    Dataf(iTrial).TarPathAng(2,1:length(iFrame)) = iFrame-Dataf(iTrial).TimeSeq(2);
...
save('eve01_110823_PreProcessed1')