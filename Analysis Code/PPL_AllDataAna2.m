...
if ismember(SecNum,SecPlots)
    ...
    SaveName = [];
    SaveName = 'SacEndErr_Sac_2E_XY_Norm_BootS_Shuff_Marked_Comp_2Shuff_AllSubj';
    for iCond = CondI
        nexttile;
        if iCond == 5
...
if ismember(SecNum,SecPlots)
    ...
    set(gcf,'Position',[31,228,1486,712]);
    SaveName = [];
    SaveName = 'SacEndErr_Sac_2E_XY_Norm_BootS_Shuff_Marked_Comp_AllSubj';
...
%% 4 compare the tar axis and sac axis difference
SecNum = SecNum+1;
if ismember(SecNum,SecPlots)
    iFigAcc = iFigAcc+1;
    figure(iFigAcc)
    set(gcf,'Position',[31,228,1486,712]);
    SaveName = [];
    SaveName = 'SacEndErr_Sac_Tar_2E_XY_Norm_BootS_Shuff_AllSubj_Ave';
    SampXAveAveAllAve_2 = [];
    SampYAveAveAllAve_2 = []; SampYAveAveAllStd_2 = []; SampYAveAveAllSE_2 = [];
    for iCond = CondI
        nexttile;
        if iCond == 5
            set(gca, 'Visible', 'off'); % This hides the (2,1) tile
        ...
        for iSubj = 1:SubjSize
            ...
            SacEndErrAngBootS_TarAxis = DataAll1(iSubj).sbdBoots.SacEndErrAngBootS_TarAxis;
            SacEndErrAngShuff_TarAxis = DataAll1(iSubj).sbdBoots.SacEndErrAngShuff_TarAxis;

            SampXAveAve = []; SampYAveAve = []; Samp2Shuff95 = []; ShuffYAveAve = [];
            % load the bootstrap result
            SampXAveAve = []; SampYAveAve = [];
            SampXAveAve = SacEndErrAngBootS_TarAxis(iCond).SampXAveAve;
            SampYAveAve = SacEndErrAngBootS_TarAxis(iCond).SampYAveAve;
            % Load the comparision result
            Samp2Shuff95 = SacEndErrAngShuff_TarAxis(iCond).Samp2Shuff95;
            % load the shuff result for the future comparision
            ShuffYAveAve = SacEndErrAngShuff_TarAxis(iCond).ShuffYAveAve;
        ...
        % plot all subject's data for sac axis
        % SampXAveAveAllAve_1 and SampYAveAveAllAve_1 have been defined before
        % use boundedline to plot which can also skip the nan point
        [hl1,hp1] = boundedline(SampXAveAveAllAve_1(iCond,:),SampYAveAveAllAve_1(iCond,:),SampYAveAveAllSE_1(iCond,:));
        set(hl1,'color',colorRGB2(iCond,:),'LineStyle','-','LineWidth',3);
        set(hp1,'FaceColor',colorRGB1(iCond,:),'FaceAlpha',0.4,'EdgeColor','none')

        % plot all subject's data for tar axis
        SampXAveAveAllAve_2(iCond,:) = wrapTo2Pi(circ_mean_nan(SampXAveAveAll));
        SampYAveAveAllAve_2(iCond,:) = circ_mean_nan(SampYAveAveAll);
        SampYAveAveAllStd_2(iCond,:) = circ_std_nan(SampYAveAveAll);
        SampYAveAveAllSE_2(iCond,:) = SampYAveAveAllStd_2(iCond,:)/sqrt(SubjSize);
        % use boundedline to plot which can also skip the nan point
        [hl2,hp2] = boundedline(SampXAveAveAllAve_2(iCond,:),SampYAveAveAllAve_2(iCond,:),SampYAveAveAllSE_2(iCond,:));
        set(hl2,'color',colorRGB2(iCond,:),'LineStyle','--','LineWidth',3);
        set(hp2,'FaceColor',colorRGB1(iCond,:),'FaceAlpha',0.4,'EdgeColor','none')
        ...
        if iCond == 1
            xlabel('Saccade Direction at Sacc/Tar End')
            ylabel('De-Sta_Trend Sacc-End Direction Difference')
            legend([hl1,hl2],{'Sac Axis','Tar Axis'},'Box','off','AutoUpdate','off')
        end
        xline(pi/2,'LineWidth',1.5,'LineStyle',':');
        xline(pi,'LineWidth',1.5,'LineStyle',':');
        xline(3*pi/2,'LineWidth',1.5,'LineStyle',':');
...
%% 5 plot all the mean value together for all subjects and compare between moving direction
SecNum = SecNum+1;
if ismember(SecNum,SecPlots)
    iFigAcc = iFigAcc+1;
    figure(iFigAcc)
    set(gcf,'Position',[31,228,1486,712]);
    SaveName = [];
    SaveName = 'SacEndErr_Sac_Tar_2E_XY_Norm_BootS_Shuff_Comp_AllSubj';
    ...
    for iDir = 1:2
        ...
        for iCond = CondIComp(iDir,:)
            ...
            if iDir == 2 && iCond == 1
                ...
                [hl2,hp] = boundedline(SampXAveAveAllAve_2(iCond,:),-SampYAveAveAllAve_2(iCond,:),SampYAveAveAllSE_2(iCond,:));
