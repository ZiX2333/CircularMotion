...
% adjusted on Oct 04, exclude several figures
% Data Save in Result/ Oct04
...
%% Target location with Reaction Time
...
SaveName{1} = '/SacRT_TarLocGocOn_';
SaveName{2} = '/SacRT_TarLocSacOnN100_';
SaveName{3} = '/SacRT_TarLocSacOn_';
SaveName{4} = '/SacRT_TarLocSacOff_';
...
for iFig = 1:4
    ...
    sgtitle(['Relation Between Reaction Time and Target Location at ',TiteName{iFig}],'FontSize',15)
...
SaveName{1} = '/SacCur_TarLocGocOn_';
SaveName{2} = '/SacCur_TarLocSacOnN100_';
SaveName{3} = '/SacCur_TarLocSacOn_';
SaveName{4} = '/SacCur_TarLocSacOff_';
...
SaveName{1} = '/SacIniErrNoSign_TarLocGocOn_';
SaveName{2} = '/SacIniErrNoSign_TarLocSacOnN100_';
SaveName{3} = '/SacIniErrNoSign_TarLocSacOn_';
SaveName{4} = '/SacIniErrNoSign_TarLocSacOff_';
...
for iFig = 1:4
    ...
    for iCond = CondI
        ...
        xlim([Xlim1,Xlim2])
        if iCond == 0
            ylabel('Saccade Initial Error, deg')
            xlabel('Target Location')
    ...
    sgtitle(['Relation Between Saccade Initial Error and Target Location at ',TiteName{iFig}],'FontSize',15)
    saveas(gcf,[ResultDir,SaveName{iFig},CondName,'.fig'])
end
iFigAcc = iFigAcc+iFig;
...
SaveName{1} = '/SacEndErrNoSign1_TarLocGocOn_';
SaveName{2} = '/SacEndErrNoSign1_TarLocSacOnN100_';
SaveName{3} = '/SacEndErrNoSign1_TarLocSacOn_';
SaveName{4} = '/SacEndErrNoSign1_TarLocSacOff_';
...
for iFig = 1:4
    ...
    sgtitle(['Relation Between Saccade (Tangental) Ending Error and Target Location at ',TiteName{iFig}],'FontSize',15)
    saveas(gcf,[ResultDir,SaveName{iFig},CondName,'.fig'])
end
...
for iFig = 1:4
    ...
    for iCond = CondI
        ...
        % plot(XG_med,YG_ave,1.5,colorRGB2(i,:),'LineStyle','-');
        % add the std fill
        fill([XG_med,fliplr(XG_med)],[YG_ave-YG_std,fliplr(YG_ave+YG_std)],colorRGB1(i,:),'EdgeColor','none','FaceAlpha',0.3);
        % plot the real dot point
        % scatter(ProcX,ProcY,20,colorRGB(i,:),'filled');
        % mark the mean velue point
        plot(XG_med,YG_ave,'Marker','x','MarkerSize',10,'Color','k','LineWidth',2);
