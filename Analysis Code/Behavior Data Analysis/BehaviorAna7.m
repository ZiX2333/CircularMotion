...
sbd.SmPLVelGoc11 = NaN(size(DatafN)); % velocity [50ms, 120ms] after the first saccade before the second
sbd.SmPLVelGoc12 = NaN(size(DatafN)); % velocity [120ms, 200ms]
sbd.SmPLVelGoc13 = NaN(size(DatafN)); % velocity [200ms, 300ms]
...
for iTrial = 1:size(DatafN,2)
    ...
    % if the trial end before 50ms or end before 120 (if end before 120 then must before 50)
    if sbd.SacETmGoc1(iTrial)+50 >= size(DatafN(iTrial).EyeLocRAcc,2) ||...
            sbd.SacETmGoc1(iTrial)+120 >= size(DatafN(iTrial).EyeLocRAcc,2)||...
            sbd.SacETmGoc1(iTrial)+200 >= size(DatafN(iTrial).EyeLocRAcc,2)
        continue
    end
    % if the first saccade blink then also continue
    if ismember(iTrial,iDropA)
        continue
    end
    % found the smp section first, for [50, 70]
    ...
    sbd.SmPLVelGoc11(iTrial) = F_SmPCalcu(SmPVSec,SmPASec,DurThrs,VelThrs,AccThrs,'linear');
    ...
    if sbd.SmPLVelGoc11(iTrial) >=50 || sbd.SmPLVelGoc11(iTrial) ==0
        sbd.SmPLVelGoc11(iTrial) = nan;
    end
    
    % Calculate the smp velocity [120,200], remove the saccade within
    SmPVSec = []; SmPASec = [];
    try
        SmPVSec = DatafN(iTrial).EyeLocRVel(6,sbd.SacETmGoc1(iTrial)+120:sbd.SacETmGoc1(iTrial)+200);
        % I may need Acc to find the saccade during smooth pursuit. I didnt
        % * 1000 in ReadEyeLoc.m
        SmPASec = DatafN(iTrial).EyeLocRAcc(6,sbd.SacETmGoc1(iTrial)+120:sbd.SacETmGoc1(iTrial)+200)*1000;
    catch % if the trial end before 120ms
        SmPVSec = DatafN(iTrial).EyeLocRVel(6,sbd.SacETmGoc1(iTrial)+120:end);
        SmPASec = DatafN(iTrial).EyeLocRAcc(6,sbd.SacETmGoc1(iTrial)+120:end)*1000;
    end
    sbd.SmPLVelGoc12(iTrial) = F_SmPCalcu(SmPVSec,SmPASec,DurThrs,VelThrs,AccThrs,'linear');
    % if the value is too large then means that it may not be the real
    % smooth pursuit
    % and velocity equals zero this is the calculation issue I need to fix
    if sbd.SmPLVelGoc12(iTrial) >=60 || sbd.SmPLVelGoc12(iTrial) ==0
        sbd.SmPLVelGoc12(iTrial) = nan;
    end
    ...
    % Calculate the smp velocity [200,300], remove the saccade within
    SmPVSec = []; SmPASec = [];
    try
        SmPVSec = DatafN(iTrial).EyeLocRVel(6,sbd.SacETmGoc1(iTrial)+200:sbd.SacETmGoc1(iTrial)+300);
        % I may need Acc to find the saccade during smooth pursuit. I didnt
        % * 1000 in ReadEyeLoc.m
        SmPASec = DatafN(iTrial).EyeLocRAcc(6,sbd.SacETmGoc1(iTrial)+200:sbd.SacETmGoc1(iTrial)+300)*1000;
    catch % if the trial end before 120ms
        SmPVSec = DatafN(iTrial).EyeLocRVel(6,sbd.SacETmGoc1(iTrial)+200:end);
        SmPASec = DatafN(iTrial).EyeLocRAcc(6,sbd.SacETmGoc1(iTrial)+200:end)*1000;
    end
    sbd.SmPLVelGoc13(iTrial) = F_SmPCalcu(SmPVSec,SmPASec,DurThrs,VelThrs,AccThrs,'linear');
    % if the value is too large then means that it may not be the real
    % smooth pursuit
    % and velocity equals zero this is the calculation issue I need to fix
    if sbd.SmPLVelGoc13(iTrial) >=60 || sbd.SmPLVelGoc13(iTrial) ==0
        sbd.SmPLVelGoc13(iTrial) = nan;
    end
end
...
CondI = [1,2,3,4,5,6,7]; % Sta % CCW % CW
% sbd.SacEndErrAng2ESign2Normed_TarNoOrder = nan(size(sbd.SacEndErrAng2ESign));
for iTime = 1:8 % GoCue, 100, 80, 60, 40, 20, Sac On, Sac Off
    for iCond = CondI
        datas1 = find([DatafN.TarDir1] == iCond & ([DatafN.TrialStatus] == 1 | [DatafN.TrialStatus] == 5));
        XV = [];YV = []; RhoVAve = []; RhoVStd = [];
        TarLocTemp = []; SacErrTemp = [];
        for iData = datas1
            TarLocTemp = [TarLocTemp,sbd.TarTMrk2E{iData}(iTime,3)];
            SacErrTemp = [SacErrTemp,sbd.SacTMrkErrAng2ESign{iData}(iTime)];
        end
        XV = mod(rad2deg(wrapTo2Pi(TarLocTemp))-90,360);
        YV = rad2deg(SacErrTemp);
        if iCond == 1
            XVbase = []; YVbase = [];
            XVbase = XV; YVbase = YV;
        end
        ...
        % Record the Origin data
        sbd.SacEndErrAng2ESign2Origin_TarTMrk{iTime,iCond} = [TarLocTemp; SacErrTemp];
        % Record the normlized data
        sbd.SacEndErrAng2ESign2Normed_TarTMrk{iTime,iCond} = [deg2rad(mod(XNorm+90,360)) ; deg2rad(YNorm)];
        sbd.SacEndErrAng2ESign2Normed_TarNoOrderTMrk{iTime,iCond} = nan(size(YNorm));
        sbd.SacEndErrAng2ESign2Normed_TarNoOrderTMrk{iTime,iCond}(XIndAll) = deg2rad(YNorm);
    end
end
