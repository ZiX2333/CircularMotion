...
sbd.SmPVelGoc11 = NaN(size(DatafN)); % velocity [50ms, 120ms] after the first saccade before the second
sbd.SmPVelGoc12 = NaN(size(DatafN)); % velocity [120ms, 200ms]
sbd.SmPVelGoc13 = NaN(size(DatafN)); % velocity [200ms, 300ms]
...
for iTrial = 1:size(DatafN,2)
    ...
    sbd.SmPVelGoc11(iTrial) = F_SmPCalcu(SmPVSec,SmPASec,DurThrs,VelThrs,AccThrs,'linear');
    ...
    if sbd.SmPVelGoc11(iTrial) >=50 || sbd.SmPVelGoc11(iTrial) ==0
        sbd.SmPVelGoc11(iTrial) = nan;
    end
    ...
    sbd.SmPVelGoc12(iTrial) = F_SmPCalcu(SmPVSec,SmPASec,DurThrs,VelThrs,AccThrs,'linear');
    % if the value is too large then means that it may not be the real
    % smooth pursuit
    % and velocity equals zero this is the calculation issue I need to fix
    if sbd.SmPVelGoc12(iTrial) >=60 || sbd.SmPVelGoc12(iTrial) ==0
        sbd.SmPVelGoc12(iTrial) = nan;
    end
    ...
    sbd.SmPVelGoc13(iTrial) = F_SmPCalcu(SmPVSec,SmPASec,DurThrs,VelThrs,AccThrs,'linear');
    % if the value is too large then means that it may not be the real
    % smooth pursuit
    % and velocity equals zero this is the calculation issue I need to fix
    if sbd.SmPVelGoc13(iTrial) >=60 || sbd.SmPVelGoc13(iTrial) ==0
        sbd.SmPVelGoc13(iTrial) = nan;
    end
end
