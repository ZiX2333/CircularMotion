...
for iTrial = 1:size(Dataf1,2)
    ...
    sbd.TarPath{iTrial}(1,:) = (Dataf1(iTrial).TarPathXReal(1,:) - Dataf1(iTrial).center(1))/Dataf1(iTrial).ppd(1); %X
    sbd.TarPath{iTrial}(2,:) = (Dataf1(iTrial).TarPathYReal(1,:) - Dataf1(iTrial).center(2))/Dataf1(iTrial).ppd(2); %Y
    [sbd.TarPath{iTrial}(3,:),sbd.TarPath{iTrial}(4,:)] = cart2pol(sbd.TarPath{iTrial}(1,:),sbd.TarPath{iTrial}(2,:));
    sbd.TarPath{iTrial}(5,:) = Dataf1(iTrial).TarPathXReal(2,:);
...
for iTrial = 1:size(Dataf1,2)
    ...
    [sbd.TarPath2E{iTrial}(3,:),sbd.TarPath2E{iTrial}(4,:)] = ...
        cart2pol(sbd.TarPath2E{iTrial}(1,:),sbd.TarPath2E{iTrial}(2,:)); % r and theta location
