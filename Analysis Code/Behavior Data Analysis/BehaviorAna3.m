...
% Ten edition on May 13 Xuan
    % add the normalized ending error calculation
...
for iTrial = 1:size(DatafN,2)
    ...
    SacEndLoc = mean(sbd.SacTraGoc1{iTrial}(1:2,end-2:end),2); % 3 ms range
    ...
    SacIniLoc = mean(sbd.SacTraGoc1{iTrial}(1:2,1:3),2); % to target ending location
...
sbd.EyeCartGoc2E = cell(size(DatafN)); %2E to the eye initial location
sbd.EyePolrGoc2E = cell(size(DatafN)); %2E to the eye initial location
...
for iTrial = 1:size(DatafN,2)
    ...
    sbd.TarGocEnd2E{iTrial}(:,1:2) = [DatafN(iTrial).SacTarGoc1(end,1)-SacIniLoc(1,1),...
        DatafN(iTrial).SacTarGoc1(end,2)-SacIniLoc(2,1)];
    [sbd.TarGocEnd2E{iTrial}(:,3),sbd.TarGocEnd2E{iTrial}(:,4)] = cart2pol(sbd.TarGocEnd2E{iTrial}(1),sbd.TarGocEnd2E{iTrial}(2));
    ...
    sbd.SacDynErrAngTan{iTrial}  = wrapToPi(sbd.EyePolrGocTan{iTrial} - sbd.TarGocEnd2E{iTrial}(3));
    
    % I will give a three ms time range
    if iniT >= sbd.SacDurGoc1(iTrial)
        sbd.SacIniDir(iTrial) = nan; % Saccade initial direction
        sbd.SacAllDir(iTrial) = wrapToPi(circ_mean(sbd.EyePolrGoc2E{iTrial}(1,end-2:end)')); % Saccade Overall direction

    else
        sbd.SacIniDir(iTrial) = wrapToPi(circ_mean(sbd.EyePolrGoc2E{iTrial}(1,iniT-1:iniT+1)')); % Saccade initial direction
        sbd.SacAllDir(iTrial) = wrapToPi(circ_mean(sbd.EyePolrGoc2E{iTrial}(1,end-2:end)')); % Saccade Overall direction
    end

    sbd.SacIniErrAng2E(iTrial) = wrapToPi(sbd.SacIniDir(iTrial) - sbd.TarGocEnd2E{iTrial}(3));
    sbd.SacEndErrAng2E(iTrial) = wrapToPi(sbd.SacAllDir(iTrial) - sbd.TarGocEnd2E{iTrial}(3));
    % 
    if DatafN(iTrial).TarDir1 >= 5 && DatafN(iTrial).TarDir1 <=7 % at CW reverse the direction
        ...
        sbd.SacIniErrAng2ESign(iTrial) = sbd.SacIniErrAng2E(iTrial);
        sbd.SacEndErrAng2ESign(iTrial) = sbd.SacIniErrAng2E(iTrial);
    end
...
%% Calculate the normalized ending error
...
for iCond = CondI
    ...
    sbd.SacEndErrAng2CSign2Origin{iCond} = [sbd.TarEndTR(1,datas1); sbd.SacEndErrAng2CSign2(datas1)];
