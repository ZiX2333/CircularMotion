...
%% 1 plot the eye traces 2E with the link between, and color code with RT?
SecNum = SecNum+1;
if ismember(SecNum,SecPlots)
    ...
    t = tiledlayout('flow');
    LLimRT = 100; ULimRT = 300; % upper and lower limit of RT
    RTZS = (sbd.SacRTGoc1 - LLimRT) / (ULimRT - LLimRT); % Quadratic scaling
    for iCond = CondI
        nexttile
        if iCond == 5
        ...
        datas1 = find([Dataf1.TarDir1] == iCond & ([Dataf1.TrialStatus] == 1 | [Dataf1.TrialStatus] == 5) &...
            sbd.SacRTGoc1 >= LLimRT & sbd.SacRTGoc1 <= ULimRT);
        Colors = turbo(length(datas1)); % get the value from color map
        for iTrial = datas1
            ...
            ColorIdx = round(1 + (size(Colors, 1) - 1) * RTZS(iTrial));
            p1 = [];
            s1 = [];
            ...
            % Polar Plot
            p1 = polarplot(EyeLoc(3,:),EyeLoc(4,:),'LineWidth',1,'Color','k');
            ...
            polarplot([EyeLoc(3,end),TarLocTemp(1)],[EyeLoc(4,end),TarLocTemp(2)],...
                'LineWidth',1.5,'Color',Colors(ColorIdx, :),'LineStyle','-');
        end
        rlim([0,12])
...
%% 2 Smooth pursuit velocity as a function of time seperated by RT
SecNum = SecNum+1;
if ismember(SecNum,SecPlots)
    iFigAcc = iFigAcc+1;
    figure(iFigAcc)
    set(gcf,'Position',[31,228,1486,712]);
    SaveName = [];
    SaveName = '/PostSaccSmpLVel_RT';

    lStyle = {'--','-.',':','-'};
    
    GrpIdx = cell(1,length(CondI));
    RTGrpAve = cell(1,length(CondI));
    ...
    for iCond = CondI
        ...
        hold on
        hl = []; hp = [];
        for iGrp = 1:length(RTEdge)-1
            GrpIdx{iCond}{iGrp} = find(RT>=RTEdge(iGrp) & RT<=RTEdge(iGrp+1));
            RTGrpAve{iCond}(iGrp) = mean(RT(GrpIdx{iCond}{iGrp}),'omitmissing');
        ...
        legend({['LQ: ',num2str(RTGrpAve{iCond}(1),'%.2f')],...
            ['HQ: ',num2str(RTGrpAve{iCond}(4),'%.2f')]},'Box','off','Location','southeast','FontSize',15);
...
%% 3 Smooth pursuit velocity as a function of time seperated by RT
SecNum = SecNum+1;
if ismember(SecNum,SecPlots)
    iFigAcc = iFigAcc+1;
    figure(iFigAcc)
    set(gcf,'Position',[31,228,1486,712]);
    SaveName = [];
    SaveName = '/PostSaccSmpLVel_Delay';

    lStyle = {'--','-.',':','-'};
    
    GrpIdx = cell(1,length(CondI));
    DelayGrpAve = cell(1,length(CondI));
    SmPLVelAllGrp = cell(1,length(CondI));
    SmPLVelAllGrpAve = cell(1,length(CondI));
    SmPLVelAllGrpStd = cell(1,length(CondI));
    ...
    for iCond = CondI
        ...
        Delay = [Dataf1(datas1).DurDelay];
        SmPLVelAll = sbd.SmPLVelAllGoc1(datas1,:);
        SmPTime = 1:size(SmPLVelAll,2);

        % seperate the EndErr into four group
        DelayEdge = quantile(Delay,0:0.25:1);
        
        GrpIdx{iCond} = cell(1,4);
        DelayGrpAve{iCond} = nan(1,4);
        SmPLVelAllGrp{iCond} = cell(1,4);
        SmPLVelAllGrpAve{iCond} = nan(4,500);
        SmPLVelAllGrpStd{iCond} = nan(4,500);
        ...
        hold on
        hl = []; hp = [];
        for iGrp = 1:length(DelayEdge)-1
            GrpIdx{iCond}{iGrp} = find(Delay>=DelayEdge(iGrp) & Delay<=DelayEdge(iGrp+1));
            DelayGrpAve{iCond}(iGrp) = mean(Delay(GrpIdx{iCond}{iGrp}),'omitmissing');
            SmPLVelAllGrp{iCond}{iGrp} = SmPLVelAll(GrpIdx{iCond}{iGrp},:);
            SmPLVelAllGrpAve{iCond}(iGrp,:) = mean(SmPLVelAllGrp{iCond}{iGrp},'omitmissing');
            SmPLVelAllGrpStd{iCond}(iGrp,:) = std(SmPLVelAllGrp{iCond}{iGrp},'omitmissing');
        ...
        legend({['LQ: ',num2str(DelayGrpAve{iCond}(1),'%.2f')],...
            ['HQ: ',num2str(DelayGrpAve{iCond}(4),'%.2f')]},'Box','off','Location','southeast','FontSize',15);
...
if ismember(SecNum,SecPlots)
    ...
    for iCond = CondI
        ...
        % find all trials
        datas = find([Dataf1.TarDir1] == iCond);
        % find all droped trials
        datas2 = find([Dataf1.TarDir1] == iCond & ~isnan([Dataf1.TimeGocOn]));
        ...
        for iTrial = datas
            ...
            TarLocGoCX(iTriali) = (double(Sti.PathXs{iTrial}(GoCueFrame))- Dataf1(iTrial).center(1))/Dataf1(iTrial).ppd(1);
            TarLocGocY(iTriali) = (double(Sti.PathYs{iTrial}(GoCueFrame))- Dataf1(iTrial).center(2))/Dataf1(iTrial).ppd(2);
            [TarLocGocT(iTriali),TarLocGocR(iTriali)] = cart2pol(TarLocGoCX(iTriali),TarLocGocY(iTriali));

        end
        polarhistogram(TarLocGocT,8,'FaceColor',colorRGB(iCond,:))
        hold on
        ...
        polarhistogram(TarLocTemp(:,1),8,'FaceColor',colorRGB1(iCond,:),'FaceAlpha',0.2,'EdgeAlpha',0.2)
        hold off
