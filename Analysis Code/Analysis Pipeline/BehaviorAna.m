...
iniT1 = 10; %  for 25% of ending error traces rotate to saccade onset
iniT2 = 20; %  for curvature analysis
...
% 24th edition on Aug 12 2025
    % change the way of calculate the smooth pursuit
% 25th edition on Sep 24 2025
    % add the angular lcoation of eye
% 25th edition on Oct 15 2025
    % unwrap the flipped target location at saccade end
    % calculate de stationary trend of curvature, for targ and eye
% 26th edition on Dec 15, 2025
    % check the saccade amplitude as a function of target speed and saccade
    % and target direction
% 27th edition on Dec 29 2025
    % add the amplitude error calculation
% 28th edition on Jan 26 2026
    % flip the amp error sign, add amp err as a function of eye and targ
    % location
% 29th edition on Feb 06 2026
    % change the same speed curvature calculation
...
% assign the space for the velocity traces
LenfSacOff = 500;
sbd.RhoSacOff = NaN(size(DatafN,2),LenfSacOff); % radius from saccade onset
sbd.AngVelSacOff = NaN(size(DatafN,2),LenfSacOff); % angular velocity from saccade offset
sbd.LinVelSacOff = NaN(size(DatafN,2),LenfSacOff); % linear velocity from saccade offset, ang * Rho
sbd.LinAccSacOff = NaN(size(DatafN,2),LenfSacOff); % linear velocity from saccade offset, ang * Rho
sbd.RhoVelSacOff = NaN(size(DatafN,2),LenfSacOff); % radius velocity from saccade offset

sbd.RhoSmPurs = NaN(size(DatafN,2),LenfSacOff); % radius from saccade onset
sbd.AngVelSmPurs = NaN(size(DatafN,2),LenfSacOff); % angular velocity from saccade offset
sbd.LinVelSmPurs = NaN(size(DatafN,2),LenfSacOff); % linear velocity from saccade offset, ang * Rho
sbd.LinAccSmPurs = NaN(size(DatafN,2),LenfSacOff); % linear velocity from saccade offset, ang * Rho
sbd.RhoVelSmPurs = NaN(size(DatafN,2),LenfSacOff); % radius velocity from saccade offset
...
for iTrial = 1:size(DatafN,2)
    ...
    %%% calculate the linear velocity align to saccade on
    % input the radius, ang vel, linear vel, and rho vel
    % LenfSacOff = length(DatafN(iTrial).EyeLocRVel(1,sbd.SacSTmGoc1(iTrial):end));
    EyeX = DatafN(iTrial).EyeLocR(1,:);
    EyeY = DatafN(iTrial).EyeLocR(2,:);
    EyeRho = DatafN(iTrial).EyeLocR(4,:);
    EyeAng = DatafN(iTrial).EyeLocR(3,:);
    EyeAngVel = zeros(size(EyeRho));
    for iTime = 3: length(EyeAng)-2
        EyeAngVel(iTime) = (angle(exp(1i*(EyeAng(iTime+2) - EyeAng(iTime-2)))) +...
            angle(exp(1i*(EyeAng(iTime+1) - EyeAng(iTime-1)))))/6 * 1000;
    end
    EyeLinVel = EyeAngVel .* EyeRho;
    EyeLinAcc = [0,diff(EyeLinVel) .* 1000];
    EyeRhoVel = DatafN(iTrial).EyeLocRVel(4,:);
    
    % measure the length
    if sum(EyeRho(sbd.SacETmGoc1(iTrial)+1:end) > 30) ==0 %if no blink after saccade
        % check the new length align to saccade offset
        StartT = sbd.SacETmGoc1(iTrial)-200;
        LenfSacOffR = length(DatafN(iTrial).EyeLocR(4,StartT:end));
        % align according to saccade off
        n = min(LenfSacOff, LenfSacOffR);
        sbd.XSacOff(iTrial,1:n) = EyeX(StartT : StartT+n-1);
        sbd.YSacOff(iTrial,1:n) = EyeY(StartT : StartT+n-1);
        sbd.RhoSacOff(iTrial,1:n) = EyeRho(StartT : StartT+n-1);
        sbd.AngSacOff(iTrial,1:n) = EyeAng(StartT : StartT+n-1);
        sbd.AngVelSacOff(iTrial,1:n) = EyeAngVel(StartT : StartT+n-1);
        sbd.LinVelSacOff(iTrial,1:n) = EyeLinVel(StartT : StartT+n-1);
        sbd.RhoVelSacOff(iTrial,1:n) = EyeRhoVel(StartT : StartT+n-1);
        sbd.LinAccSacOff(iTrial,1:n) = EyeLinAcc(StartT : StartT+n-1);

        % calculate post-saccade pursuit vel
        StartT = sbd.SacETmGoc1(iTrial)+1;
        LenfSacOffR = length(DatafN(iTrial).EyeLocR(4,StartT:end));
        n = min(LenfSacOff, LenfSacOffR);
        sbd.XSmPurs(iTrial,1:n) = EyeX(StartT : StartT+n-1);
        sbd.YSmPurs(iTrial,1:n) = EyeY(StartT : StartT+n-1);
        sbd.RhoSmPurs(iTrial,1:n) = EyeRho(StartT : StartT+n-1);
        sbd.AngSmPurs(iTrial,1:n) = EyeAng(StartT : StartT+n-1);
        sbd.AngVelSmPurs(iTrial,1:n) = EyeAngVel(StartT : StartT+n-1);
        sbd.LinVelSmPurs(iTrial,1:n) = EyeLinVel(StartT : StartT+n-1);
        sbd.RhoVelSmPurs(iTrial,1:n) = EyeRhoVel(StartT : StartT+n-1);
        sbd.LinAccSmPurs(iTrial,1:n) = EyeLinAcc(StartT : StartT+n-1);
    end
...
for iTrial = 1:size(DatafN,2)
    ...
    sbd.LinVelSmPurs(iTrial,:) = ...
        F_SmPDeSacc(sbd.LinVelSmPurs(iTrial,:),sbd.LinAccSmPurs(iTrial,:),DurThrs,VelThrs,AccThrs,VelAvd);
    % also for LinVelSacOff?
end
...
% add saccade amplitude error
sbd.TarAmp2E = NaN(1,length(DatafN));
sbd.SacAmpErr1 = NaN(1,length(DatafN)); % amplitudes difference
sbd.SacAmpErr2 = NaN(1,length(DatafN)); % End location distance difference
sbd.SacIniDir = NaN(size(DatafN)); % Saccade initial direction at 10ms
sbd.SacIniDir2 = NaN(size(DatafN)); % Saccade initial direction at 20ms
sbd.SacAllDir = NaN(size(DatafN)); % Saccade Overall direction
sbd.SacCurPara_IniDir = NaN(size(DatafN)); % first curvature parameters: All dir - Ini Dir
...
for iTrial = 1:size(DatafN,2)
    ...
    [t,r] = cart2pol(sbd.TarTMrk2E{iTrial}(:,1),sbd.TarTMrk2E{iTrial}(:,2));
    sbd.TarTMrk2E{iTrial} = [sbd.TarTMrk2E{iTrial},t,r];
    
    % target amp 2E and Sac amplitude error
    sbd.TarAmp2E(iTrial) = sbd.TarEnd2E(4,iTrial);
    % amplitude difference, not the distance between ends
    sbd.SacAmpErr1(iTrial) = sbd.SacAmpGoc1(iTrial) - sbd.TarAmp2E(iTrial); 
    % distance between ends
    sbd.SacAmpErr2(iTrial) = norm(sbd.SacIni2E(1:2,iTrial) - sbd.TarEnd2E(1:2,iTrial));
    ...
    % I will give a three ms time range, for iniT at 10ms
    if iniT1 >= sbd.SacDurGoc1(iTrial)
        sbd.SacIniDir(iTrial) = nan; % Saccade initial direction
    else
        sbd.SacIniDir(iTrial) = wrapToPi(circ_mean(sbd.SacTraGoc2E1{iTrial}(3,iniT1-1:iniT1+1)')); % Saccade initial direction
    end
    % I will give a three ms time range, for iniT at 20ms
    if iniT2 >= sbd.SacDurGoc1(iTrial)
        sbd.SacIniDir2(iTrial) = nan; % Saccade initial direction
    else
        sbd.SacIniDir2(iTrial) = wrapToPi(circ_mean(sbd.SacTraGoc2E1{iTrial}(3,iniT2-1:iniT2+1)')); % Saccade initial direction
    end
    ...
    sbd.SacCurPara_IniDir(iTrial) = wrapToPi(sbd.SacAllDir(iTrial)-sbd.SacIniDir2(iTrial));
end
...
sbd.SacCurPara_PPD = NaN(size(DatafN)); % use prependicular distance method to calculate
sbd.SacCurPara_PPDTime = NaN(size(DatafN)); % Saccade Maxium curvatue duration
sbd.SacCurPara_PPD_Amp = NaN(size(DatafN)); % maximum ppd / saccade amplitude
sbd.SacCurPara_PPDDir = NaN(size(DatafN)); % curvature direction, CCW+1, CW-1
sbd.SacCurPara_PPD_AmpDir = NaN(size(DatafN)); % SacCurPara_PPD_Amp.*SacCurPara_PPDDir
...
for iTrial = 1:size(DatafN,2)
    ...
    % velocity mark
    EyeTrace = sbd.SacTraGoc1{iTrial};
    SacIniLoc = sbd.SacIniXY(1:2,iTrial);
    SacEndLoc = sbd.SacEndXY(1:2,iTrial);
    SaccAmp = sbd.SacAmpGoc1(iTrial);
    SacEndDir = sbd.SacAllDir(iTrial);

    % Calculate the equation of the line segment formed by trace 1
    a = SacIniLoc(2) - SacEndLoc(2); % y1-y2
    b = SacEndLoc(1) - SacIniLoc(1); % x2-x1
    c = SacIniLoc(1) * SacEndLoc(2) - SacEndLoc(1) * SacIniLoc(2); % x1*y2 - x2*y1
    perpendicular_distance = [];
    % Loop through each point in trace2 and calculate perpendicular distances
    for i = 1:size(EyeTrace,2)
        x = EyeTrace(1,i);
        y = EyeTrace(2,i);
        % Calculate perpendicular distance to the line segment
        perpendicular_distance(i) = abs(a * x + b * y + c) / sqrt(a^2 + b^2);
    end

    [ppdcMax,MaxIdx] = max(perpendicular_distance);
    sbd.SacCurPara_PPD(iTrial) = ppdcMax;
    sbd.SacCurPara_PPD_Amp(iTrial) = ppdcMax/SaccAmp;
    sbd.SacCurPara_PPDTime(iTrial) = MaxIdx;
    % check if the max perpendicular location is on the CCW or CW direction
    MaxLocDir = EyeTrace(3,MaxIdx);
    if wrapToPi(MaxLocDir - SacEndDir) <0 % CCW
        sbd.SacCurPara_PPDDir(iTrial) = 1;
    elseif wrapToPi(MaxLocDir - SacEndDir) >0 % CW
        sbd.SacCurPara_PPDDir(iTrial) = -1;
    elseif wrapToPi(MaxLocDir - SacEndDir) ==0 % straight line
        sbd.SacCurPara_PPDDir(iTrial) = 0;
    end
    sbd.SacCurPara_PPD_AmpDir(iTrial) = sbd.SacCurPara_PPDDir(iTrial) * sbd.SacCurPara_PPD_Amp(iTrial);
end
...
sbd.TarEnd2EAngWrap = mod(wrapTo2Pi(sbd.TarEnd2E(3,:))-pi/2,pi*2); %rotate the zero to vertical up
...
sbd.TarEnd2EAngWrapflip(datas1) = 2*pi - sbd.TarEnd2EAngWrap(datas1); % flip the CW and wrap

% unwrap the wrap on the flipped data
sbd.TarEnd2EAngflip = mod(wrapTo2Pi(sbd.TarEnd2E(3,:))+pi/2,pi*2); %flip, and unwrap

%% wrapped and flipped saccade end 2E and unwrapped
SacEnd2EAngWrap = mod(wrapTo2Pi(sbd.SacEnd2E(3,:))-pi/2, pi*2);% rotate the zero to vertical up
sbd.SacEnd2EAngWrap = SacEnd2EAngWrap;

% flip the CW direction
datas1 = find([DatafN.TarDir1] == 5 | [DatafN.TarDir1] == 6 | [DatafN.TarDir1] == 7);
SacEnd2EAngWrap(datas1) = 2*pi - SacEnd2EAngWrap(datas1); 
sbd.SacEnd2EAngWrapFlip = SacEnd2EAngWrap; % flip the CW and wrap

% unwrap the wrap on the flipped data
SacEnd2EAngFlip = mod(SacEnd2EAngWrap + pi/2, pi*2);
sbd.SacEnd2EAngFlip = SacEnd2EAngFlip; % flip the CW and unwrap
...
sbd.SacAngErr2ESignDeStaT = NaN(size(DatafN));
...
for iCond = CondI
    ...
    sbd.SacAngErr2ESignDeStaT(datas1) = YNorm;
end

%% Ending error zero means
% zero means, under each condition
sbd.SacAngErr2ESignDeStaTCen1 = NaN(size(DatafN)); 
for iCond = CondI
    datas1 = find([DatafN.TarDir1] == iCond & ([DatafN.TrialStatus] == 1 | [DatafN.TrialStatus] == 5));
    YV = sbd.SacAngErr2ESignDeStaT(datas1);
    YVCen = YV - circ_mean_nan(YV');
    sbd.SacAngErr2ESignDeStaTCen1(datas1) = YVCen;
end

% zero means, under same speed
sbd.SacAngErr2ESignDeStaTCen2 = NaN(size(DatafN)); 
...
for iCond = CondIComp(1,:)
    ...
    YV = sbd.SacAngErr2ESignDeStaT(datas1);
    YVCen = YV - circ_mean_nan(YV');
    sbd.SacAngErr2ESignDeStaTCen2(datas1) = YVCen;
end

%% Calculate the de-stationary-trend Amp Err, x axis as target location
% Remove stationary trend amp error 2E, x axis as target location
sbd.SacAmpErr1DeStaT = NaN(size(DatafN));
CondI = [1,2,3,4,5,6,7]; % Sta % CCW % CW
winSize = pi/6; % 30 deg
stepSize = winSize;
% sbd.SacEndErrAng2ESign2Normed_TarNoOrder = nan(size(sbd.SacEndErrAng2ESign));
for iCond = CondI
    datas1 = find([DatafN.TarDir1] == iCond & ([DatafN.TrialStatus] == 1 | [DatafN.TrialStatus] == 5));
    XV = mod(wrapTo2Pi(sbd.TarEnd2E(3,datas1))-pi/2,2*pi);
    YV = sbd.SacAmpErr1(datas1);
    if iCond == 1
        XVbase = XV; YVbase = YV;
    end
    % do the normalization (this one don't need to flip Sta dir for CW)
    [~, YNorm, ~] = F_CartScaMovNorm1(winSize,stepSize,XV',XVbase',YV',YVbase',[0,2*pi-winSize]);

    % Record the Origin data
    sbd.SacAmpErr1DeStaT(datas1) = YNorm;
end

%% Amp Error Zero Means
% zero means, under each condition
sbd.SacAmpErr1DeStaTCen1 = NaN(size(DatafN)); 
for iCond = CondI
    datas1 = find([DatafN.TarDir1] == iCond & ([DatafN.TrialStatus] == 1 | [DatafN.TrialStatus] == 5));
    YV = sbd.SacAmpErr1DeStaT(datas1);
    YVCen = YV - mean(YV,'omitmissing');
    sbd.SacAmpErr1DeStaTCen1(datas1) = YVCen;
end

% zero means, under same speed
sbd.SacAmpErr1DeStaTCen2 = NaN(size(DatafN)); 
CondIComp = [1,2,3,4; 1,5,6,7];
for iCond = CondIComp(1,:)
    datas1 = find(([DatafN.TarDir1] == iCond | [DatafN.TarDir1] == CondIComp(2,iCond)) ...
        & ([DatafN.TrialStatus] == 1 | [DatafN.TrialStatus] == 5));
    YV = sbd.SacAmpErr1DeStaT(datas1);
    YVCen = YV - mean(YV,'omitmissing');
    sbd.SacAmpErr1DeStaTCen2(datas1) = YVCen;
end

%% Calculate the de-stationary-trend ending error 2E, x axis aas Eye location 
% Remove stationary trend ending error 2E, x axis as target location 
sbd.SacAngErr2ESignDeStaE = NaN(size(DatafN));
CondI = [1,2,3,4,5,6,7]; % Sta % CCW % CW
winSize = pi/6; % 30 deg
stepSize = winSize;
% sbd.SacEndErrAng2ESign2Normed_TarNoOrder = nan(size(sbd.SacEndErrAng2ESign));
for iCond = CondI
    ...
    % Record the Origin data
    sbd.SacAngErr2ESignDeStaE(datas1) = YNorm;
end

%% Ending error zero means
% zero means, under each condition
sbd.SacAngErr2ESignDeStaECen1 = NaN(size(DatafN)); 
for iCond = CondI
    datas1 = find([DatafN.TarDir1] == iCond & ([DatafN.TrialStatus] == 1 | [DatafN.TrialStatus] == 5));
    YV = sbd.SacAngErr2ESignDeStaE(datas1);
    YVCen = YV - circ_mean_nan(YV');
    sbd.SacAngErr2ESignDeStaECen1(datas1) = YVCen;
end

% zero means, under same speed
sbd.SacAngErr2ESignDeStaECen2 = NaN(size(DatafN)); 
CondIComp = [1,2,3,4; 1,5,6,7];
for iCond = CondIComp(1,:)
    datas1 = find(([DatafN.TarDir1] == iCond | [DatafN.TarDir1] == CondIComp(2,iCond)) ...
        & ([DatafN.TrialStatus] == 1 | [DatafN.TrialStatus] == 5));
    YV = sbd.SacAngErr2ESignDeStaE(datas1);
    YVCen = YV - circ_mean_nan(YV');
    sbd.SacAngErr2ESignDeStaECen2(datas1) = YVCen;
end

%% Calculate the de-stationary-trend Amp Err, x axis as Eye location
% Remove stationary trend amp error 2E, x axis as target location
sbd.SacAmpErr1DeStaE = NaN(size(DatafN));
CondI = [1,2,3,4,5,6,7]; % Sta % CCW % CW
winSize = pi/6; % 30 deg
stepSize = winSize;
% sbd.SacEndErrAng2ESign2Normed_TarNoOrder = nan(size(sbd.SacEndErrAng2ESign));
for iCond = CondI
    ...
    % Record the Origin data
    sbd.SacAmpErr1DeStaE(datas1) = YNorm;
end

%% centralization for amp err
% zero means, under each condition
sbd.SacAmpErr1DeStaECen1 = NaN(size(DatafN)); 
for iCond = CondI
    datas1 = find([DatafN.TarDir1] == iCond & ([DatafN.TrialStatus] == 1 | [DatafN.TrialStatus] == 5));
    YV = sbd.SacAmpErr1DeStaE(datas1);
    YVCen = YV - mean(YV,'omitmissing');
    sbd.SacAmpErr1DeStaECen1(datas1) = YVCen;
end

% zero means, under same speed
sbd.SacAmpErr1DeStaECen2 = NaN(size(DatafN)); 
CondIComp = [1,2,3,4; 1,5,6,7];
for iCond = CondIComp(1,:)
    datas1 = find(([DatafN.TarDir1] == iCond | [DatafN.TarDir1] == CondIComp(2,iCond)) ...
        & ([DatafN.TrialStatus] == 1 | [DatafN.TrialStatus] == 5));
    YV = sbd.SacAmpErr1DeStaE(datas1);
    YVCen = YV - mean(YV,'omitmissing');
    sbd.SacAmpErr1DeStaECen2(datas1) = YVCen;
end

%% Calculate the de-stationary-trend RT, x axis aas target location 
% Remove stationary trend ending error 2E, x axis as target location 
sbd.SacRTGoc1DeStaT = NaN(size(DatafN));
CondI = [1,2,3,4,5,6,7]; % Sta % CCW % CW
winSize = pi/6; % 30 deg
stepSize = winSize;
% sbd.SacEndErrAng2ESign2Normed_TarNoOrder = nan(size(sbd.SacEndErrAng2ESign));
for iCond = CondI
    ...
    % Record the Origin data
    sbd.SacRTGoc1DeStaT(datas1) = YNorm;
end

%% Calculate the de-statioanry-trend RT zero means, centralization
sbd.SacRTGoc1DeStaTCen1 = NaN(size(DatafN));
for iCond = CondI
    datas1 = find([DatafN.TarDir1] == iCond & ([DatafN.TrialStatus] == 1 | [DatafN.TrialStatus] == 5));
    YV = sbd.SacRTGoc1DeStaT(datas1);
    YVCen = YV - mean(YV,'omitmissing');
    sbd.SacRTGoc1DeStaTCen1(datas1) = YVCen;
end

% zero means, under same speed
sbd.SacRTGoc1DeStaTCen2 = NaN(size(DatafN)); 
CondIComp = [1,2,3,4; 1,5,6,7];
for iCond = CondIComp(1,:)
    datas1 = find(([DatafN.TarDir1] == iCond | [DatafN.TarDir1] == CondIComp(2,iCond)) ...
        & ([DatafN.TrialStatus] == 1 | [DatafN.TrialStatus] == 5));
    YV = sbd.SacRTGoc1DeStaT(datas1);
    YVCen = YV - mean(YV,'omitmissing');
    sbd.SacRTGoc1DeStaTCen2(datas1) = YVCen;
end

%% Calculate the de-stationary-trend RT, x axis aas Eye location 
% Remove stationary trend ending error 2E, x axis as target location 
sbd.SacRTGoc1DeStaE = NaN(size(DatafN));
CondI = [1,2,3,4,5,6,7]; % Sta % CCW % CW
winSize = pi/6; % 30 deg
stepSize = winSize;
% sbd.SacEndErrAng2ESign2Normed_TarNoOrder = nan(size(sbd.SacEndErrAng2ESign));
for iCond = CondI
    ...
    % Record the Origin data
    sbd.SacRTGoc1DeStaE(datas1) = YNorm;
end

%% Calculate the de-statioanry-trend RT zero means for Eye location, centralization
sbd.SacRTGoc1DeStaECen1 = NaN(size(DatafN));
for iCond = CondI
    datas1 = find([DatafN.TarDir1] == iCond & ([DatafN.TrialStatus] == 1 | [DatafN.TrialStatus] == 5));
    YV = sbd.SacRTGoc1DeStaE(datas1);
    YVCen = YV - mean(YV,'omitmissing');
    sbd.SacRTGoc1DeStaECen1(datas1) = YVCen;
end

% zero means, under same speed
sbd.SacRTGoc1DeStaECen2 = NaN(size(DatafN)); 
CondIComp = [1,2,3,4; 1,5,6,7];
for iCond = CondIComp(1,:)
    datas1 = find(([DatafN.TarDir1] == iCond | [DatafN.TarDir1] == CondIComp(2,iCond)) ...
        & ([DatafN.TrialStatus] == 1 | [DatafN.TrialStatus] == 5));
    YV = sbd.SacRTGoc1DeStaE(datas1);
    YVCen = YV - mean(YV,'omitmissing');
    sbd.SacRTGoc1DeStaECen2(datas1) = YVCen;
end


%% calculate the de-stationary trend curvature for target location
% Remove stationary trend for (PPD/AMP) * Dir
sbd.SacCurPara_PPD_AmpDirDeStaT = NaN(size(DatafN));
% Remove stationary trend for all dir - ini dir
sbd.SacCurPara_IniDirDeStaT = NaN(size(DatafN));
CondI = [1,2,3,4,5,6,7]; % Sta % CCW % CW
winSize = pi/6; % 15 deg
stepSize = winSize;
% sbd.SacEndErrAng2ESign2Normed_TarNoOrder = nan(size(sbd.SacEndErrAng2ESign));
for iCond = CondI
    datas1 = find([DatafN.TarDir1] == iCond & ([DatafN.TrialStatus] == 1 | [DatafN.TrialStatus] == 5));
    XV = mod(wrapTo2Pi(sbd.TarEnd2E(3,datas1))-pi/2,2*pi);
    YV1 = sbd.SacCurPara_PPD_AmpDir(datas1);
    YV2 = sbd.SacCurPara_IniDir(datas1);
    if iCond == 1
        XVbase = XV; YVbase1 = YV1; YVbase2 = YV2;
    end
    % do the normalization, don't need to flip the CW
    [~, YNorm1, ~] = F_CartScaMovNorm1(winSize,stepSize,XV',XVbase',YV1',YVbase1',[0,2*pi-winSize]);
    [~, YNorm2, ~] = F_CartScaMovNorm(winSize,stepSize,XV',XVbase',YV2',YVbase2',[0,2*pi-winSize]);

    sbd.SacCurPara_PPD_AmpDirDeStaT(datas1) = YNorm1;
    sbd.SacCurPara_IniDirDeStaT(datas1) = YNorm2;
end

%% Calculate the de-statioanry-trend Curvature zero means, centralization
sbd.SacCurPara_PPD_AmpDirDeStaTCen1 = NaN(size(DatafN));
sbd.SacCurPara_IniDirDeStaTCen1 = NaN(size(DatafN));
for iCond = CondI
    datas1 = find([DatafN.TarDir1] == iCond & ([DatafN.TrialStatus] == 1 | [DatafN.TrialStatus] == 5));
    YV1 = sbd.SacCurPara_PPD_AmpDirDeStaT(datas1);
    YV2 = sbd.SacCurPara_IniDirDeStaT(datas1);
    YVCen1 = YV1 - mean(YV1,'omitmissing');
    YVCen2 = YV2 - circ_mean_nan(YV2');
    sbd.SacCurPara_PPD_AmpDirDeStaTCen1(datas1) = YVCen1;
    sbd.SacCurPara_IniDirDeStaTCen1(datas1) = YVCen2;
end

% zero means, under same speed
sbd.SacCurPara_PPD_AmpDirDeStaTCen2 = NaN(size(DatafN)); 
sbd.SacCurPara_IniDirDeStaTCen2 = NaN(size(DatafN)); 
CondIComp = [1,2,3,4; 1,5,6,7];
for iCond = CondIComp(1,:)
    datas1 = find([DatafN.TarDir1] == iCond & ([DatafN.TrialStatus] == 1 | [DatafN.TrialStatus] == 5));
    datas2 = find([DatafN.TarDir1] == CondIComp(2,iCond) & ([DatafN.TrialStatus] == 1 | [DatafN.TrialStatus] == 5));
    % datas1 = find(([DatafN.TarDir1] == iCond | [DatafN.TarDir1] == CondIComp(2,iCond)) ...
    %     & ([DatafN.TrialStatus] == 1 | [DatafN.TrialStatus] == 5));
    if iCond == 1
        YV1 = sbd.SacCurPara_PPD_AmpDirDeStaT(datas1);
        YV2 = sbd.SacCurPara_IniDirDeStaT(datas1);
    else % need to flip CW sign to combine
        YV1 = [sbd.SacCurPara_PPD_AmpDirDeStaT(datas1), -sbd.SacCurPara_PPD_AmpDirDeStaT(datas2)];
        YV2 = [sbd.SacCurPara_IniDirDeStaT(datas1), -sbd.SacCurPara_IniDirDeStaT(datas2)];
    ...
    YVCen1 = YV1 - mean(YV1,'omitmissing');
    YVCen2 = YV2 - circ_mean_nan(YV2');
    sbd.SacCurPara_PPD_AmpDirDeStaTCen2(datas1) = YVCen1;
    sbd.SacCurPara_IniDirDeStaTCen2(datas1) = YVCen2;
end

%% calculate the de-stationary trend curvature for eye location
% Remove stationary trend for (PPD/AMP) * Dir
sbd.SacCurPara_PPD_AmpDirDeStaE = NaN(size(DatafN));
% Remove stationary trend for all dir - ini dir
sbd.SacCurPara_IniDirDeStaE = NaN(size(DatafN));
CondI = [1,2,3,4,5,6,7]; % Sta % CCW % CW
winSize = pi/6; % 15 deg
stepSize = winSize;
% sbd.SacEndErrAng2ESign2Normed_TarNoOrder = nan(size(sbd.SacEndErrAng2ESign));
for iCond = CondI
    ...
    sbd.SacCurPara_PPD_AmpDirDeStaE(datas1) = YNorm1;
    sbd.SacCurPara_IniDirDeStaE(datas1) = YNorm2;
end

%% Calculate the de-statioanry-trend Curvature zero means for eye location, centralization
sbd.SacCurPara_PPD_AmpDirDeStaECen1 = NaN(size(DatafN));
sbd.SacCurPara_IniDirDeStaECen1 = NaN(size(DatafN));
for iCond = CondI
    datas1 = find([DatafN.TarDir1] == iCond & ([DatafN.TrialStatus] == 1 | [DatafN.TrialStatus] == 5));
    YV1 = sbd.SacCurPara_PPD_AmpDirDeStaE(datas1);
    YV2 = sbd.SacCurPara_IniDirDeStaE(datas1);
    YVCen1 = YV1 - mean(YV1,'omitmissing');
    YVCen2 = YV2 - circ_mean_nan(YV2');
    sbd.SacCurPara_PPD_AmpDirDeStaECen1(datas1) = YVCen1;
    sbd.SacCurPara_IniDirDeStaECen1(datas1) = YVCen2;
end

% zero means, under same speed
sbd.SacCurPara_PPD_AmpDirDeStaECen2 = NaN(size(DatafN)); 
sbd.SacCurPara_IniDirDeStaECen2 = NaN(size(DatafN)); 
CondIComp = [1,2,3,4; 1,5,6,7];
for iCond = CondIComp(1,:)
    datas1 = find([DatafN.TarDir1] == iCond & ([DatafN.TrialStatus] == 1 | [DatafN.TrialStatus] == 5));
    datas2 = find([DatafN.TarDir1] == CondIComp(2,iCond) & ([DatafN.TrialStatus] == 1 | [DatafN.TrialStatus] == 5));
    ...
    if iCond == 1
        ...
        YV1 = [sbd.SacCurPara_PPD_AmpDirDeStaE(datas1), -sbd.SacCurPara_PPD_AmpDirDeStaE(datas2)];
        YV2 = [sbd.SacCurPara_IniDirDeStaE(datas1), -sbd.SacCurPara_IniDirDeStaE(datas2)];
        datas1 = [datas1, datas2];
    end
    % YV1 = sbd.SacCurPara_PPD_AmpDirDeStaT(datas1);
    % YV2 = sbd.SacCurPara_IniDirDeStaT(datas1);
    YVCen1 = YV1 - mean(YV1,'omitmissing');
    YVCen2 = YV2 - circ_mean_nan(YV2');
    sbd.SacCurPara_PPD_AmpDirDeStaECen2(datas1) = YVCen1;
    sbd.SacCurPara_IniDirDeStaECen2(datas1) = YVCen2;
end
...
for iCond = CondI
    ...
    vfEstimateEyeEnd(iCond,:) = circ_ksdensity(EyeEndTta, vfPDFSamples, [0, 2*pi], fSigma);
end
sbd.vfPDFSamples = vfPDFSamples;
sbd.vfEstimateEyeEnd = vfEstimateEyeEnd;

%% Add the KDE analysis to target direction at saccade end
vfEstimateTarEnd = [];
CondI = [1,2,3,4,5,6,7]; % Sta % CCW % CW
for iCond = CondI
    datas1 = [];
    datas1 = find([DatafN.TarDir1] == iCond & ([DatafN.TrialStatus] == 1 | [DatafN.TrialStatus] == 5));
    TarEndTta = zeros(size(datas1));
    TarEndTta = wrapTo2Pi(sbd.TarEnd2E(3,datas1));
    vfPDFSamples = 0:StepSZ:2*pi;
    vfEstimateTarEnd(iCond,:) = circ_ksdensity(TarEndTta, vfPDFSamples, [0, 2*pi], fSigma);
end
sbd.vfEstimateTarEnd = vfEstimateTarEnd;

%% Add the KDE analysis to target direction at Goc
vfEstimateTarGoc = [];
CondI = [1,2,3,4,5,6,7]; % Sta % CCW % CW
for iCond = CondI
    datas1 = [];
    datas1 = find([DatafN.TarDir1] == iCond & ([DatafN.TrialStatus] == 1 | [DatafN.TrialStatus] == 5));
    TarGocTta = zeros(size(datas1));
    TarGocTta = wrapTo2Pi(sbd.TarGoc2E(3,datas1));
    vfPDFSamples = 0:StepSZ:2*pi;
    vfEstimateTarGoc(iCond,:) = circ_ksdensity(TarGocTta, vfPDFSamples, [0, 2*pi], fSigma);
end
sbd.vfEstimateTar = vfEstimateTarGoc;
