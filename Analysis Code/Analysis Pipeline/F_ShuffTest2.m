function SacEndErrAngShuff = F_ShuffTest2(inputXV,inputYV,ReTime,winSize,stepSize,winRange)
...
for iReTime = 1:ReTime
    ...
    [XAve, YAve, YStd] = F_CartScaSlidWin_PolData1(winSize,stepSize,inputXV,ShuffYV,winRange);
...
for iShuffY = 1:size(ShuffYAve,2)
    ...
    ShuffYAveAve(iShuffY) = mean(ShuffYAve(:,iShuffY),'omitmissing');
    ...
    ShuffYStdErr(iShuffY) = std(ShuffYAve(:,iShuffY),'omitmissing');
end
