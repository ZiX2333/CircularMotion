function ellipse_t = F_FitEllipse( x,y,OKplot )
%
% This function is adjusted from fit_ellipse
% Citation: Ohad Gal (2024). fit_ellipse (https://www.mathworks.com/matlabcentral/fileexchange/3215-fit_ellipse),
%           MATLAB Central File Exchange. Retrieved May 13, 2024.
% Updated on Apr 22, add the different color input Xuan, remove the axis
% handle
% the inpput example before: ellipse_t = fit_ellipse( x,y,axis_handle )
% Updated on May 13, add the output of the ellipse and choose the plotting
% option, also added xaligned_ellipse (based on Patrick's code)
% Updated on May 15, add the angXtoL, always output the angle between X
% axis and Long axis, counterclockwise tilting >0, clockwise tilting <0
% Updated on May 22, remove the detection of tilting or not, just get the
% tilted orientation anyway;
...
%
% The angle phi is the counterclockwise angle between
% the original x-axis and the new x-axis (or u-axis) after rotation.
% A positive phi indicates a counterclockwise rotation.
% A negative phi indicate a clockwise rotation.
%
...
%       F`` = -F` + (D`^2)/(4*A`) + (E`^2)/(4*C`)
%
%       Such that:    a`*(X-X0)^2 = A`(X^2 + Xorientation_tolerance*D`/A` + (D`/(2*A`))^2 )
...
%   Orientation = phi
%
% Plot Option: 0 no plots, 1 plots
%
% Set the output color
col = 'k';

% initialize
% if the coefficient of the term "x*y" is not zero. could also be 1e-3
% orientation_tolerance = 1e-2*5;
% no orientation checking anymore
...
if (test>0)
    ...
    long_axis   = 2*max(a,b);
    short_axis  = 2*min(a,b);
    if a>=b && orientation_rad~=0
        angXtoL = -orientation_rad;
    elseif a<b && orientation_rad~=0
        angXtoL = (pi/2-abs(orientation_rad))*sign(orientation_rad);
    end
...
% draw if plot option is 1
if OKplot
    % hold_state = get( axis_handle,'NextPlot' );
    % set( axis_handle,'NextPlot','add' );
    plot( new_ver_line(1,:),new_ver_line(2,:), col);
    plot( new_horz_line(1,:),new_horz_line(2,:),col );
    plot( rotated_ellipse(1,:),rotated_ellipse(2,:),col,'LineWidth',1.5);
end
...
ellipse_t.new_ver_line = new_ver_line;
ellipse_t.new_horz_line = new_horz_line;
ellipse_t.R = R;
end
