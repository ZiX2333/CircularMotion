...
LenfSacOn = 400;
sbd.RhoSacOn = NaN(size(DatafN,2),LenfSacOn); % radius from saccade onset
sbd.AngVelSacOn = NaN(size(DatafN,2),LenfSacOn); % angular velocity from saccade onset
sbd.LinVelSacOn = NaN(size(DatafN,2),LenfSacOn); % linear velocity from saccade onset, ang * Rho
sbd.LinAccSacOn = NaN(size(DatafN,2),LenfSacOn); % linear velocity from saccade onset, ang * Rho
sbd.RhoVelSacOn = NaN(size(DatafN,2),LenfSacOn); % radius velocity from saccade onset
...
for iTrial = 1:size(DatafN,2)
    ...
    % LenfSacOn = length(DatafN(iTrial).EyeLocRVel(1,sbd.SacSTmGoc1(iTrial):end));
    ...
    EyeLinAcc = diff(EyeLinVel) .* 1000;
    EyeRhoVel = DatafN(iTrial).EyeLocRVel(4,:);
    
    % measure the length
    if sum(EyeRho(sbd.SacETmGoc1(iTrial):end) > 30) ==0 %if no blink after saccade
        % check the new length align to saccade onset
        StartT = sbd.SacSTmGoc1(iTrial)-50;
        LenfSacOnR = length(DatafN(iTrial).EyeLocR(4,StartT:end));
        % align according to saccade on
        n = min(LenfSacOn, LenfSacOnR);
        sbd.RhoSacOn(iTrial,1:n) = EyeRho(StartT : StartT+n);
        sbd.AngVelSacOn(iTrial,1:n) = EyeAngVel(StartT : StartT+n);
        sbd.LinVelSacOn(iTrial,1:n) = EyeLinVel(StartT : StartT+n);
        sbd.RhoVelSacOn(iTrial,1:n) = EyeRhoVel(StartT : StartT+n);
        sbd.LinAccSacOn(iTrial,1:n) = EyeLinAcc(StartT : StartT+n);

        % align according to saccade off
        % the coding makes me want to die!!!!!!

        % calculate post-saccade pursuit vel
        if sbd.SacETmGoc1(iTrial)+500 <= length(DatafN(iTrial).EyeLocRVel(6,:))
            sbd.SmPLVelAllGoc1(iTrial,:) = EyeVelAcc(sbd.SacETmGoc1(iTrial)+1:sbd.SacETmGoc1(iTrial)+500);
            sbd.SmPLAccAllGoc1(iTrial,:) = EyeAccAcc(sbd.SacETmGoc1(iTrial)+1:sbd.SacETmGoc1(iTrial)+500)*1000;
            % calculate for the radius velocity transfer to angular velocity
            % linear speed to r*ang vel
            % if blink then remove this trial
            if sum(RhoLoc(sbd.SacETmGoc1(iTrial)+1:sbd.SacETmGoc1(iTrial)+500) > 30) >=1
                continue
            end
            sbd.SmPA2LVelAllGoc1(iTrial,:) = EyeVelAng(sbd.SacETmGoc1(iTrial)+1:sbd.SacETmGoc1(iTrial)+500)...
                .*RhoLoc(sbd.SacETmGoc1(iTrial)+1:sbd.SacETmGoc1(iTrial)+500);
            ...
            TrueLen = length(DatafN(iTrial).EyeLocRAcc(6,:))-(sbd.SacETmGoc1(iTrial));
            sbd.SmPLVelAllGoc1(iTrial,1:TrueLen) = EyeVelAcc(sbd.SacETmGoc1(iTrial)+1:end);
            sbd.SmPLAccAllGoc1(iTrial,1:TrueLen) = EyeAccAcc(sbd.SacETmGoc1(iTrial)+1:end)*1000;
            % calculate for the radius velocity transfer to angular velocity
            % linear speed to r*ang vel
            % if blink then remove this trial
            if sum(RhoLoc(sbd.SacETmGoc1(iTrial)+1:end) > 30)>=1
                continue
            end
            sbd.SmPA2LVelAllGoc1(iTrial,1:TrueLen) = ...
                EyeVelAng(sbd.SacETmGoc1(iTrial)+1:end).*RhoLoc(sbd.SacETmGoc1(iTrial)+1:end);
            SmPA2LAccAllGoc1 = diff(sbd.SmPA2LVelAllGoc1(iTrial,1:TrueLen))*1000;
            SmPA2LAccAllGoc1 = [SmPA2LAccAllGoc1,SmPA2LAccAllGoc1(end)];
            sbd.SmPA2LAccAllGoc1(iTrial,1:TrueLen) = SmPA2LAccAllGoc1;
        end
    end
...
SacEnd2EAngWrap = mod(wrapTo2Pi(sbd.SacEnd2E(3,:))-pi/2, pi*2);
datas1 = find([DatafN.TarDir1] == 5 | [DatafN.TarDir1] == 6 | [DatafN.TarDir1] == 7);
SacEnd2EAngWrap(datas1) = 2*pi - SacEnd2EAngWrap(datas1);
SacEnd2EAngFlip = mod(SacEnd2EAngWrap + pi/2, pi*2);
sbd.SacEnd2EAngFlip = SacEnd2EAngFlip;
sbd.SacEnd2EAngWrapFlip = SacEnd2EAngWrap;

%% flip the saccade direction
% will do it when next time loop through all subjects
