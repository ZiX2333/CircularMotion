...
Sti.SmPurDur = 400; % following smooth pursuit time
Sti.size = 0.5;
...
Sti.DisRange = [-25,0; 0,-25; -16,-5];
Sti.DirMove = [1,-1]; % moving inward or outward
Sti.Speed = 30; %30 deg/s
Sti.WindowSize = 20; % diameter
    ...
    for i = 1:num_trial
        ...
        if  rem(num_CondSec(i,iBlock),3) == 1 % inward
            for j = 1:(FP.OffFrames(i,iBlock)+Sti.SEnFrames+Sti.SacFrames+Sti.SmPFrames)
                ...
                % limit location
                if CurrDis + Sti.Speed*ifi <= Sti.DisRange(i,2)
                    CurrDis = CurrDis + Sti.Speed*ifi;
                elseif CurrDis + Sti.Speed*ifi > Sti.DisRange(i,2)
                    CurrDis = CurrDis;
                end
            end
